...
export class EntityMigrationTracker {
  ...
  trackPerformance(architecture, operation, duration, metadata = {}) {
    try {
      ...
      if (!architecture || !operation || typeof duration !== "number") {
        console.warn(
          "[EntityMigrationTracker] Invalid performance tracking parameters",
        );
        return;
      }
      ...
      if (
        ...
        console.log(
          `[EntityMigrationTracker] ${this.entityType}.${operation} (${architecture}): ${duration.toFixed(2)}ms`,
        );
      }
      ...
      console.error(
        "[EntityMigrationTracker] Failed to track performance:",
        error,
      );
    }
  }
    ...
    try {
      ...
      if (this.errorDetails[architecture]) {
        ...
        if (this.errorDetails[architecture].length > 100) {
          this.errorDetails[architecture] =
            this.errorDetails[architecture].slice(-100);
        }
      }

      console.error(
        `[EntityMigrationTracker] ${this.entityType}.${operation} (${architecture}) error:`,
        error,
      );
      ...
      console.error(
        "[EntityMigrationTracker] Failed to track error:",
        trackingError,
      );
    }
  }
  ...
  trackUserInteraction(architecture, interaction, responseTime, details = {}) {
    try {
      ...
      this.trackPerformance(architecture, "interaction", responseTime, {
        interaction,
        details,
      });
      ...
      if (this.userBehavior.clickPatterns.length > 500) {
        this.userBehavior.clickPatterns =
          this.userBehavior.clickPatterns.slice(-500);
      }

      console.log(
        `[EntityMigrationTracker] User interaction (${architecture}): ${interaction} - ${responseTime.toFixed(2)}ms`,
      );
    } catch (error) {
      console.error(
        "[EntityMigrationTracker] Failed to track user interaction:",
        error,
      );
    }
  }
  ...
  trackUserSatisfaction(architecture, rating, feedback = "", context = {}) {
    try {
      ...
      console.log(
        `[EntityMigrationTracker] User satisfaction (${architecture}): ${rating}/5 - "${feedback}"`,
      );
    } catch (error) {
      console.error(
        "[EntityMigrationTracker] Failed to track user satisfaction:",
        error,
      );
    }
  }
  ...
  configureABTesting(config = {}) {
    ...
    if (this.abTesting.enabled) {
      ...
      this.abTesting.currentArchitecture =
        randomValue < this.abTesting.trafficSplit ? "legacy" : "new";
    }

    console.log(
      "[EntityMigrationTracker] A/B testing configured:",
      this.abTesting,
    );
  }
  ...
  generatePerformanceReport(reportType = "summary") {
    try {
      ...
      Object.keys(this.metrics).forEach((metricKey) => {
        if (
          typeof this.metrics[metricKey] === "object" &&
          this.metrics[metricKey].legacy &&
          this.metrics[metricKey].new
        ) {
          report.metrics[metricKey] = this._generateMetricReport(
            metricKey,
            reportType,
          );
        }
      ...
      console.log(
        `[EntityMigrationTracker] Generated ${reportType} performance report`,
      );

      return report;
    } catch (error) {
      console.error(
        "[EntityMigrationTracker] Failed to generate performance report:",
        error,
      );
      return null;
    }
  ...
  getMigrationReadiness() {
    try {
      ...
      assessment.recommendations = this._getReadinessRecommendations(
        assessment.scores,
      );

      console.log(
        `[EntityMigrationTracker] Migration readiness: ${assessment.overall} (${overallScore.toFixed(1)}%)`,
      );

      return assessment;
    } catch (error) {
      console.error(
        "[EntityMigrationTracker] Failed to assess migration readiness:",
        error,
      );
  ...
  _initializeTracking() {
    try {
      ...
      console.error(
        "[EntityMigrationTracker] Failed to initialize tracking:",
        error,
      );
    }
  }
    ...
    try {
      if (performance.memory) {
        ...
        if (this.metrics.memoryUsage[architecture].length > 100) {
          this.metrics.memoryUsage[architecture] =
            this.metrics.memoryUsage[architecture].slice(-100);
        }
      }
    } catch (error) {
  ...
  _checkPerformanceTarget(metricKey, architecture, value) {
    ...
    if (target && value > target * 1.1) {
      // 10% tolerance
      console.warn(
        `[EntityMigrationTracker] Performance target missed for ${metricKey} (${architecture}): ${value}ms > ${target}ms`,
      );
    }
  }
  ...
  _checkErrorTarget(architecture) {
    ...
    if (target && current > target) {
      console.warn(
        `[EntityMigrationTracker] Error rate target exceeded for ${architecture}: ${current}% > ${target}%`,
      );
    }
  }
  ...
  _checkSatisfactionTarget(architecture) {
    ...
    if (entries.length > 10) {
      ...
      if (target && average < (target / 5) * 100) {
        // Convert to percentage
        console.warn(
          `[EntityMigrationTracker] Satisfaction target not met for ${architecture}: ${average.toFixed(1)} < ${target}%`,
        );
      }
    }
  }
  ...
  _generateMetricReport(metricKey, reportType) {
    ...
    if (report.legacy.count > 0 && report.new.count > 0) {
      report.comparison = {
        improvement:
          (
            ((report.legacy.average - report.new.average) /
              report.legacy.average) *
            100
          ).toFixed(2) + "%",
        significantDifference: this._isSignificantDifference(
          metric.legacy,
          metric.new,
        ),
  ...
  _analyzeMetricData(data) {
    ...
    const values = data.map(
      (entry) => entry.duration || entry.rating || entry.used || 0,
    );
    values.sort((a, b) => a - b);
  ...
  _assessPerformanceReadiness() {
    ...
    ["loadTime", "createTime", "updateTime", "deleteTime"].forEach(
      (metricKey) => {
        const metric = this.metrics[metricKey];
  ...
  _assessSatisfactionReadiness() {
    ...
    return data.average >= target
      ? 100
      : Math.max(0, (data.average / target) * 100);
  }
  ...
  _assessMemoryReadiness() {
    ...
    const increase =
      ((newData.average - legacyData.average) / legacyData.average) * 100;
    const target = this.metrics.memoryUsage.targets?.new || 15;

    return increase <= target
      ? 100
      : Math.max(0, 100 - (increase - target) * 2);
  }
  ...
  _getReadinessRecommendations(scores) {
    ...
    if (scores.performance < 75) {
      recommendations.push(
        "Optimize performance bottlenecks before full migration",
      );
    }
    ...
    if (scores.userSatisfaction < 75) {
      recommendations.push(
        "Gather more user feedback and address satisfaction concerns",
      );
    }
    if (scores.memory < 75) {
  ...
  _sendReport(report) {
    try {
      ...
      if (window.UMIGServices?.analyticsService) {
        window.UMIGServices.analyticsService.track(
      ...
      // Log locally
      console.log(
        "[EntityMigrationTracker] Performance report generated:",
        report,
      );
    } catch (error) {
  ...
  _initializeMemoryMonitoring() {
    ...
    console.log(
      "[EntityMigrationTracker] Memory monitoring initialized with cleanup support",
    );
  }
  ...
  _initializeNetworkMonitoring() {
    ...
    console.log(
      "[EntityMigrationTracker] Network monitoring initialized with cleanup support",
    );
  }
  ...
  _initializeUserBehaviorTracking() {
    ...
    const clickHandler = (event) => {
      if (
        event.target.closest('[data-entity-type="' + this.entityType + '"]')
      ) {
    ...
    console.log(
      "[EntityMigrationTracker] User behavior tracking initialized with cleanup support",
    );
  }
  ...
  destroy() {
    console.log(
      `[EntityMigrationTracker] Destroying tracker for ${this.entityType}`,
    );
    ...
    if (this.eventListeners && this.eventListeners.length > 0) {
      this.eventListeners.forEach(({ element, event, handler }) => {
        try {
          ...
          console.warn(
            "[EntityMigrationTracker] Failed to remove event listener:",
            error,
          );
        }
      });
    ...
    try {
      ...
      console.error(
        "[EntityMigrationTracker] Failed to generate final report:",
        error,
      );
    }
  ...
  _clearAllTimers() {
    ...
    try {
      ...
      console.debug(
        "[EntityMigrationTracker] Timer cleanup completed with minor issues (expected)",
      );
    }
  }
}
