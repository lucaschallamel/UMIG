...
(function() {
    ...
    class PerformanceMonitor {
         ...
         */
        establishBaselines() {
            const operations = ['teamsLoad', 'usersLoad', 'environmentsLoad',
                              'applicationsLoad', 'labelsLoad', 'migrationTypesLoad',
                              'iterationTypesLoad'];
        ...
        compareToBaseline(name, duration) {
            ...
            if (ratio >= this.config.errorThreshold) {
                console.error(`🚨 Performance degradation for ${name}: ${percentChange.toFixed(1)}% slower than baseline`);
            } else if (ratio >= this.config.warnThreshold) {
                console.warn(`⚠️ Performance warning for ${name}: ${percentChange.toFixed(1)}% slower than baseline`);
            } else if (ratio < 0.9) {
                console.log(`✅ Performance improvement for ${name}: ${Math.abs(percentChange).toFixed(1)}% faster than baseline`);
            }
        ...
        generateReport() {
            ...
            operations.forEach(op => {
                ...
                if (this.baselines[op]) {
                    ...
                    const avgDuration = this.calculateAverage(latest.map(m => m.duration));
        ...
        generateRecommendations(report) {
            ...
            if (this.baselines.memoryUsage && currentMemory > this.baselines.memoryUsage * 1.5) {
                recommendations.push({
                    severity: 'warning',
                    category: 'memory',
                    message: 'Memory usage has increased by more than 50%. Check for memory leaks.',
        ...
        logMetric(metric) {
            ...
            console.log(`${emoji} Performance [${metric.name}]: ${metric.duration.toFixed(2)}ms`);
        }
         ...
         */
        logError(metric) {
            console.error(`❌ Performance Error [${metric.name}]:`, metric.error, `Duration: ${metric.duration.toFixed(2)}ms`);
        }
        ...
        saveBaselines() {
            try {
                localStorage.setItem('umig-performance-baselines', JSON.stringify(this.baselines));
