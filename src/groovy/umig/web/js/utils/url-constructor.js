...
class UrlConstructor {
  ...
  async _performInitialization(fallbackConfig) {
    ...
    try {
      ...
      const response = await fetch(
        "/rest/scriptrunner/latest/custom/urlConfiguration",
        {
          method: "GET",
          headers: {
      ...
      console.log(
        "✅ UrlConstructor: Server configuration loaded successfully",
      );
      ...
      console.warn(
        "⚠️ UrlConstructor: Failed to fetch server configuration, using fallback",
      );
      ...
      if (this.fallbackConfig) {
        ...
        console.error(
          "❌ UrlConstructor: No fallback configuration provided, URL construction will fail",
        );
        throw error;
      }
    }
  ...
  buildStepViewUrl({ migrationCode, iterationCode, stepCode }) {
    if (!this.initialized || !this.config) {
      console.error(
        "❌ UrlConstructor: Not initialized. Call initialize() first.",
      );
      return null;
    }
...
if (typeof window !== "undefined" && window.UMIG_STEP_CONFIG) {
  document.addEventListener("DOMContentLoaded", async function () {
    try {
      ...
      if (!success) {
        console.warn(
          "⚠️ StepView: Using fallback URL configuration. Check server connectivity and system_configuration_scf table.",
        );
      }
    } catch (error) {
      console.error(
        "❌ UrlConstructor: Auto-initialization failed:",
        error.message,
      );
    }
  });
