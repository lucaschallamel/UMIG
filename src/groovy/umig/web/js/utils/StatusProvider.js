...
(function (window) {
  ...
  class StatusProvider {
    ...
    async getStatuses(entityType) {
      ...
      if (cached) {
        console.debug(
          `StatusProvider: Cache hit for ${this.sanitizeForLogging(sanitizedEntityType)}`,
        );
        return cached;
      }
      ...
      try {
        // Fetch from API
        console.debug(
          `StatusProvider: Cache miss for ${this.sanitizeForLogging(sanitizedEntityType)}, fetching from API`,
        );
        ...
        if (response && response.statuses) {
          ...
          if (response.etag && typeof response.etag === "string") {
            this.etags.set(
              sanitizedEntityType,
              this.sanitizeString(response.etag),
            );
          }
      ...
      console.warn(
        `StatusProvider: Using fallback statuses for ${this.sanitizeForLogging(sanitizedEntityType)}`,
      );
    ...
    async getDropdownOptions(entityType) {
      ...
      if (cached) {
        console.debug(
          `StatusProvider: Cache hit for dropdown options ${this.sanitizeForLogging(sanitizedEntityType)}`,
        );
        return cached;
      }
      ...
      return statuses
        .map((status) => {
          if (!status || typeof status !== "object") {
            console.warn(
              "StatusProvider: Invalid status object in generateDropdownOptions",
            );
            return null;
          }
    ...
    getFallbackStatuses(entityType) {
      ...
      if (!fallbackOptions) {
        console.warn(
          `StatusProvider: No fallback statuses for entity type ${this.sanitizeForLogging(sanitizedEntityType)}`,
        );
        return [];
      }
    ...
    async populateStatusDropdown(selectElement, entityType, currentValue) {
      ...
      const sanitizedCurrentValue = currentValue
        ? this.sanitizeString(currentValue)
        : null;
      ...
      try {
        ...
        options.forEach((option) => {
          if (!option || typeof option !== "object") {
            console.warn(
              "StatusProvider: Invalid option object in dropdown population",
            );
            return;
          }
        ...
        console.debug(
          `StatusProvider: Populated dropdown with ${options.length} options for ${this.sanitizeForLogging(sanitizedEntityType)}`,
        );
      } catch (error) {
        ...
        const fallbackOptions =
          this.FALLBACK_STATUSES[sanitizedEntityType] || [];
    ...
    validateEntityType(entityType) {
      ...
      if (!this.ALLOWED_ENTITY_TYPES.includes(sanitized)) {
        result.errors.push(
          `Invalid entity type. Must be one of: ${this.ALLOWED_ENTITY_TYPES.join(", ")}`,
        );
        return result;
      }
