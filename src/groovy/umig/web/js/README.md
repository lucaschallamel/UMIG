...
function updateUIForRole(userRole) {
    ...
    pilotElements.forEach(el => {
        el.style.display = (userRole === 'PILOT' || userRole === 'ADMIN') ? 'block' : 'none';
    });
...
const StepsAPIv2Client = {
    ...
    fetchSteps: function(forceRefresh = false) {
        ...
        return this.apiCall('/api/v2/steps')
            .then(data => {
...
const CacheManager = {
    ...
    get: function(key, ttl = 30000) { // 30 second default TTL
