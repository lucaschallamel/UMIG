...
(function() {
    ...
    const AuthenticationManager = {
        ...
        initializeLoginForm: function(userCodeInput) {
            const config = window.AdminGuiState ? window.AdminGuiState.getState().config : {};
        ...
        checkAuthStatus: function() {
            const state = window.AdminGuiState ? window.AdminGuiState.getState() : null;
        ...
        validateUserCode: function(userCode) {
            return userCode && userCode.length === 3 && /^[A-Z0-9]{3}$/.test(userCode);
        },
        ...
        createMockUser: function(userCode) {
            ...
            const isAdmin = adminTrigrams.includes(userCode) || userCode.startsWith('A');
        ...
        onLoginSuccess: function(user) {
            ...
            if (window.UiUtils) {
                window.UiUtils.showNotification(`Welcome, ${user.usr_first_name}!`, 'success');
            }
        ...
        updateNavigationVisibility: function() {
            ...
            console.log('User is admin:', user.usr_is_admin, typeof user.usr_is_admin);

            // Show sections based on user role
            if (user.usr_is_admin === true || user.usr_is_admin === 'true' || user.usr_is_admin === 1) {
