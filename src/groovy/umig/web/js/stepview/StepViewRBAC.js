...
class StepViewRBAC {
    ...
    async initialize(stepCode) {
        try {
            console.log("🔐 StepView RBAC: Initializing security system for step:", stepCode);
            ...
            if (!response.ok) {
                throw new Error(`User context API failed: ${response.status} - ${response.statusText}`);
            }
            ...
            console.log("🔐 StepView RBAC: Security system initialized successfully", {
                userId: this.userContext.userId,
                username: this.userContext.username,
                role: this.userContext.role,
                teams: this.userContext.teamMemberships?.map(t => t.teamName) || [],
                allowedActions: Object.keys(this.permissions).filter(p => this.permissions[p])
            });

            return true;

        } catch (error) {
            console.error("🚨 StepView RBAC: Security system initialization failed:", error);
            ...
            console.warn("🔐 StepView RBAC: Applied secure fallback permissions (read-only)");
            return false;
        }
    }
    ...
    hasPermission(feature) {
        ...
        if (!this.permissions || typeof this.permissions !== "object") {
            console.error("🚨 RBAC: Permissions object invalid, denying access to:", feature);
    ...
    showPermissionDeniedMessage(action) {
        ...
        const message = roleMessages[this.userContext?.role] || "Permission denied.";
    ...
    getUserDisplayInfo() {
        return {
            ...
            assignedTeams: this.userContext?.teamMemberships?.map(t => t.teamName) || [],
