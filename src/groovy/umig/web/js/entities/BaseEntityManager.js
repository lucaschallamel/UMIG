...
export class BaseEntityManager {
    ...
    constructor(config = {}) {
        ...
        console.log(`[BaseEntityManager] Initialized for entity: ${this.entityType}`);
    }
    ...
    async initialize(container, options = {}) {
        try {
            console.log(`[BaseEntityManager] Initializing ${this.entityType} entity manager`);
            ...
            console.log(`[BaseEntityManager] ${this.entityType} initialization complete`);
        } catch (error) {
            console.error(`[BaseEntityManager] Failed to initialize ${this.entityType}:`, error);
            throw error;
        }
    }
    ...
    async loadData(filters = {}, sort = null, page = 1, pageSize = 20) {
        ...
        try {
            console.log(`[BaseEntityManager] Loading ${this.entityType} data`, { filters, sort, page, pageSize });
            ...
            if (!validationResult.isValid) {
                this._trackError('load', new SecurityUtils.ValidationException(
                    `Filter validation failed: ${validationResult.errors.join(', ')}`
                ));
                throw new SecurityUtils.ValidationException('Invalid filter parameters', 'filters', filters);
            }
            ...
            // Load data via API
            const response = await this._fetchEntityData(filters, sort, page, pageSize);
            ...
            console.log(`[BaseEntityManager] ${this.entityType} data loaded in ${loadTime.toFixed(2)}ms`);
            ...
            console.error(`[BaseEntityManager] Failed to load ${this.entityType} data:`, error);
    ...
    async createEntity(data) {
        ...
        try {
            console.log(`[BaseEntityManager] Creating ${this.entityType} entity`, data);
            ...
            if (!validationResult.isValid) {
                this._trackError('create', new SecurityUtils.ValidationException(
                    `Create validation failed: ${validationResult.errors.join(', ')}`
                ));
                throw new SecurityUtils.ValidationException('Invalid entity data for creation', 'data', data);
            }

            // Additional XSS prevention layer
            const sanitizedData = SecurityUtils.preventXSS(validationResult.sanitizedData);

            // Rate limiting check
            if (!SecurityUtils.checkRateLimit(`${this.entityType}_create`, 10, 60000)) {
                SecurityUtils.logSecurityEvent('rate_limit_exceeded_create', 'warning', { 
                    entityType: this.entityType,
                    sessionId: this.securityContext?.sessionId 
                });
                throw new SecurityUtils.SecurityException('Rate limit exceeded for entity creation');
            }
            ...
            console.log(`[BaseEntityManager] ${this.entityType} entity created successfully`);
            
            return response;
        } catch (error) {
            console.error(`[BaseEntityManager] Failed to create ${this.entityType} entity:`, error);
    ...
    async updateEntity(id, data) {
        ...
        try {
            console.log(`[BaseEntityManager] Updating ${this.entityType} entity ${id}`, data);

            // COMPREHENSIVE SECURITY VALIDATION FOR UPDATE
            const validationResult = SecurityUtils.validateInput({ id, ...data }, {
                preventXSS: true,
                preventSQLInjection: true,
                sanitizeStrings: true,
                validateLength: true
            ...
            if (!validationResult.isValid) {
                this._trackError('update', new SecurityUtils.ValidationException(
                    `Update validation failed: ${validationResult.errors.join(', ')}`
                ));
                throw new SecurityUtils.ValidationException('Invalid entity data for update', 'data', { id, ...data });
            }

            // Extract sanitized ID and data
            const { id: sanitizedId, ...sanitizedData } = validationResult.sanitizedData;

            // Additional XSS prevention layer
            const finalSanitizedData = SecurityUtils.preventXSS(sanitizedData);

            // Rate limiting check
            if (!SecurityUtils.checkRateLimit(`${this.entityType}_update`, 20, 60000)) {
                SecurityUtils.logSecurityEvent('rate_limit_exceeded_update', 'warning', { 
                    entityType: this.entityType,
                    entityId: sanitizedId,
                    sessionId: this.securityContext?.sessionId 
                });
                throw new SecurityUtils.SecurityException('Rate limit exceeded for entity updates');
            }
            ...
            const response = await this._updateEntityData(sanitizedId, finalSanitizedData);
            ...
            console.log(`[BaseEntityManager] ${this.entityType} entity updated successfully`);
            
            return response;
        } catch (error) {
            console.error(`[BaseEntityManager] Failed to update ${this.entityType} entity:`, error);
    ...
    async deleteEntity(id) {
        ...
        try {
            console.log(`[BaseEntityManager] Deleting ${this.entityType} entity ${id}`);

            // COMPREHENSIVE SECURITY VALIDATION FOR DELETE
            const validationResult = SecurityUtils.validateInput({ id }, {
                preventXSS: true,
                preventSQLInjection: true,
                sanitizeStrings: true
            });
            ...
            if (!validationResult.isValid) {
                this._trackError('delete', new SecurityUtils.ValidationException(
                    `Delete validation failed: ${validationResult.errors.join(', ')}`
                ));
                throw new SecurityUtils.ValidationException('Invalid entity ID for deletion', 'id', id);
            }

            const sanitizedId = validationResult.sanitizedData.id;

            // Rate limiting check for delete operations
            if (!SecurityUtils.checkRateLimit(`${this.entityType}_delete`, 5, 60000)) {
                SecurityUtils.logSecurityEvent('rate_limit_exceeded_delete', 'warning', { 
                    entityType: this.entityType,
                    entityId: sanitizedId,
                    sessionId: this.securityContext?.sessionId 
                });
                throw new SecurityUtils.SecurityException('Rate limit exceeded for entity deletion');
            }
            ...
            console.log(`[BaseEntityManager] ${this.entityType} entity deleted successfully`);
            
            return true;
        } catch (error) {
            console.error(`[BaseEntityManager] Failed to delete ${this.entityType} entity:`, error);
    ...
    async _initializePerformanceTracking() {
        ...
        try {
            const { EntityMigrationTracker } = await import('../utils/EntityMigrationTracker.js');
            this.performanceTracker = new EntityMigrationTracker(this.entityType);
        } catch (error) {
            console.warn(`[BaseEntityManager] Performance tracker not available: ${error.message}`);
            // Create simple fallback tracker
            this.performanceTracker = {
                trackPerformance: (operation, duration) => {
                    console.log(`[Performance] ${this.entityType}.${operation}: ${duration.toFixed(2)}ms`);
                },
                trackError: (operation, error) => {
                    console.error(`[Performance] ${this.entityType}.${operation} error:`, error);
                }
            };
        }
    }
    ...
    async _checkMigrationMode() {
        try {
            ...
            console.log(`[BaseEntityManager] Migration mode for ${this.entityType}: ${this.migrationMode}`);
        } catch (error) {
            console.warn(`[BaseEntityManager] Feature flag check failed, defaulting to new architecture:`, error);
            this.migrationMode = 'new';
        }
    }
    ...
    async _initializeComponents() {
        ...
        if (this.config.filterConfig?.enabled) {
            this.filterComponent = await this.orchestrator.createComponent('filter', {
        ...
        if (this.config.paginationConfig) {
            this.paginationComponent = await this.orchestrator.createComponent('pagination', {
    ...
    async _confirmDeleteEntity(data) {
        if (confirm(`Are you sure you want to delete this ${this.entityType.slice(0, -1)}?`)) {
     ...
     */
    destroy() {
        console.log(`[BaseEntityManager] Destroying ${this.entityType} entity manager`);
