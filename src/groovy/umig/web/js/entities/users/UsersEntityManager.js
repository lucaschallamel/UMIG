...
class UsersEntityManager extends BaseEntityManager {
  constructor() {
    ...
    this.searchFields = ["usr_first_name", "usr_last_name", "usr_email", "usr_code"];
    ...
    this.relationshipsApiUrl = "/rest/scriptrunner/latest/custom/users/relationships";
  ...
  async initialize(container) {
    ...
    try {
      ...
      console.log(`[UsersEntityManager] Initialization completed in ${operationTime.toFixed(2)}ms`);
  ...
  async loadData(filters = {}) {
    ...
    try {
      ...
      console.log(`[UsersEntityManager] Loaded ${data.length} users in ${operationTime.toFixed(2)}ms`);
  ...
  async getTeamsForUser(userId, includeArchived = false) {
    ...
    try {
      ...
      console.error("[UsersEntityManager] Failed to get teams for user:", error);
  ...
  async assignToTeam(userId, teamId, role = "USER") {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to assign user to team: ${response.statusText}`);
      }
      ...
      console.error("[UsersEntityManager] Failed to assign user to team:", error);
  ...
  async removeFromTeam(userId, teamId) {
    ...
    try {
      ...
      console.error("[UsersEntityManager] Failed to remove user from team:", error);
  ...
  async updateProfile(userId, updates) {
    ...
    try {
      ...
      console.error("[UsersEntityManager] Failed to update user profile:", error);
  ...
  async batchValidateUsers(userIds) {
    ...
    try {
      const response = await fetch(
        `${this.usersApiUrl}/batch-validate`,
        {
          method: "POST",
          headers: SecurityUtils.addCSRFProtection({
            "Content-Type": "application/json"
      ...
      if (!response.ok) {
        throw new Error(`Failed to batch validate users: ${response.statusText}`);
      }
      ...
      console.log(`[UsersEntityManager] Batch validated ${userIds.length} users in ${operationTime.toFixed(2)}ms`);
      
      return results;
    } catch (error) {
      console.error("[UsersEntityManager] Failed to batch validate users:", error);
  ...
  _setupEventHandlers() {
    ...
    this.orchestrator.on("user:assignTeam", async (data) => {
      try {
        const result = await this.assignToTeam(data.userId, data.teamId, data.role);
  ...
  _trackPerformance(operationType, duration) {
    try {
      ...
      if (duration > threshold) {
        console.warn(`[UsersEntityManager] Performance warning: ${operationType} took ${duration.toFixed(2)}ms (threshold: ${threshold}ms)`);
      }
    } catch (error) {
  ...
  _trackError(operation, error) {
    try {
      ...
      console.error("[UsersEntityManager] Failed to track error:", trackingError);
    }
  }
