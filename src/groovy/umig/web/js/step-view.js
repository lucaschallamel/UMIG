...
class StepView {
    constructor() {
        this.config = window.UMIG_STEP_CONFIG || { api: { baseUrl: '/rest/scriptrunner/latest/custom' } };
    ...
    async loadStepDetails(migrationName, iterationName, stepCode, container) {
        try {
            ...
            const response = await fetch(`${this.config.api.baseUrl}/stepViewApi/instance?${queryParams}`);
    ...
    renderInstructions(instructions) {
        ...
        const rows = instructions.map((inst, index) => `
    ...
    renderImpactedTeams(teams) {
        ...
        const teamList = teams.map(team => 
            `<li>${this.escapeHtml(team.name || team)}</li>`
        ).join('');
    ...
    applyRoleBasedControls() {
        ...
        pilotElements.forEach(el => {
            el.style.display = ['PILOT', 'ADMIN'].includes(this.userRole) ? '' : 'none';
        });
    ...
    async populateStatusDropdown() {
        ...
        try {
            ...
            // Get current status
            const currentStatus = document.querySelector('.step-info')?.dataset?.currentStatus || 'PENDING';
    ...
    async handleStatusChange(event) {
        ...
        try {
            const response = await fetch(`${this.config.api.baseUrl}/steps/${this.currentStepInstanceId}/status`, {
                method: 'PUT',
                headers: {
    ...
    async handleAddComment() {
        ...
        if (!this.currentStepInstanceId) {
            this.showNotification('Unable to add comment: No step instance ID', 'error');
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`${this.config.api.baseUrl}/comments/${this.currentStepInstanceId}/comments`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async saveCommentEdit(commentId) {
        ...
        try {
            const response = await fetch(`${this.config.api.baseUrl}/comments/${commentId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
