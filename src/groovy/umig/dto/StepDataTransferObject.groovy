...
/**
 * Unified Step Data Transfer Object for US-056-A Service Layer Standardization
 * 
 * Provides consistent, type-safe data structure for Step entities across all UMIG services.
 * Eliminates format inconsistencies between EmailService, API responses, and template rendering.
 * 
...
class StepDataTransferObject {
     ...
     * @return StepDataTransferObject instance
     */
    static StepDataTransferObject fromJson(String jsonString) {
        try {
            ...
            log.error("Failed to deserialize JSON to StepDataTransferObject: ${e.message}", e)
    ...
    Map<String, Object> toTemplateMap() {
        ...
        // Recent comments for email display
        map.recentComments = comments?.take(3)?.collect { comment ->
            [
                text: comment.text ?: "",
                author: comment.authorName ?: "Unknown",
                date: comment.createdDate?.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME) ?: ""
            ]
        } ?: []
    ...
    static class Builder {
        private StepDataTransferObject dto = new StepDataTransferObject()
        ...
        StepDataTransferObject build() {
            ...
            if (errors) {
                throw new IllegalArgumentException("StepDataTransferObject validation failed: ${errors.join(', ')}")
            }
     ...
     * @return Validated StepDataTransferObject
     */
    static StepDataTransferObject create(String stepId, String stepName, String stepStatus) {
        return builder()
            .stepId(stepId)
            .stepName(stepName)
