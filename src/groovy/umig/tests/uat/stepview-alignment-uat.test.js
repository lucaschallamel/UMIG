...
test.describe('StepView UI Alignment UAT', () => {
  ...
  test('should load StepView page and verify initial state', async ({ page }) => {
  ...
  test('should verify CSS alignment with IterationView styling', async ({ page }) => {
    await page.goto(TEST_URL);
    await page.waitForLoadState('networkidle');
    ...
    if (cssLoaded) {
      ...
      console.log('⚠️ iteration-view.css not detected - styles may be inlined or differently named');
    }

    // Verify container structure and CSS classes
    const containerSelector = '.step-details-container';
    await expect(page.locator(containerSelector)).toBeVisible({ timeout: 5000 });
  ...
  test('should verify header and status elements', async ({ page }) => {
    ...
    const statusBadgeVisible = await page.locator(statusBadgeSelector).isVisible();
    ...
    if (statusBadgeVisible) {
      ...
      const altStatusSelectors = ['.status', '[data-status]', '.badge', '.label'];
  ...
  test('should verify instructions and comments sections', async ({ page }) => {
    ...
    const instructionsVisible = await page.locator(instructionsSelector).isVisible();
    
    if (instructionsVisible) {
      console.log('✅ Instructions section with class "instructions-container" found');
    } else {
      console.log('⚠️ Instructions container not found - checking alternative selectors');
      const altInstructionsSelectors = ['.instructions', '[data-instructions]', '.step-instructions', '.content'];
    ...
    if (commentsVisible) {
      ...
      console.log('⚠️ Comments container not found - checking alternative selectors');
      const altCommentsSelectors = ['.comments', '[data-comments]', '.step-comments', '.notes'];
  ...
  test('should verify PILOT role-based access control (RBAC)', async ({ page }) => {
    await page.goto(TEST_URL);
    await page.waitForLoadState('networkidle');
    ...
    const updateStatusButton = page.locator('button').filter({ hasText: /update.*status/i });
    const pilotButtons = page.locator('[data-role="PILOT"], .pilot-only, button[id*="pilot"], button[class*="pilot"]');
    ...
    if (updateStatusVisible) {
      ...
      console.log('⚠️ Update Status button not visible - checking all available buttons');
    ...
    const roleIndicator = await page.locator('[data-role="PILOT"], .role-pilot, .user-role').first();
  ...
  test('should test mobile responsiveness', async ({ page }) => {
    ...
    const container = page.locator('.step-details-container, [data-step-container], .main-content').first();
  ...
  test('should verify complete page structure and take comprehensive screenshots', async ({ page }) => {
    await page.goto(TEST_URL);
    await page.waitForLoadState('networkidle');
    ...
    const pageStructure = await page.evaluate(() => {
      ...
      const mainContent = document.querySelector('.step-details-container, [data-step-container], .main-content, body');
      return getStructure(mainContent || document.body);
    });

    console.log('🏗️ Page structure:');
    console.log(pageStructure.slice(0, 1000) + (pageStructure.length > 1000 ? '...' : ''));
...
test.afterAll(async () => {
  ...
  console.log('📁 Screenshots saved to: /Users/lucaschallamel/Documents/GitHub/UMIG/src/groovy/umig/tests/uat/screenshots/');
});