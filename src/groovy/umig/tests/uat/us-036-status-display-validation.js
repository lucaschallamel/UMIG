...
class US036StatusDisplayValidator {
  ...
  async testRoleDetectionLogic() {
    ...
    const testCases = [
      // Formal roles should NOT show static badge
      {
        role: "ADMIN",
        expectedBadge: false,
        description: "ADMIN user (formal role)",
      },
      {
        role: "PILOT",
        expectedBadge: false,
        description: "PILOT user (formal role)",
      },
      {
        role: "NORMAL",
        expectedBadge: false,
        description: "NORMAL user (formal role)",
      },
      ...
      // Non-formal roles should show static badge
      {
        role: null,
        expectedBadge: true,
        description: "null role (no formal role)",
      },
      {
        role: undefined,
        expectedBadge: true,
        description: "undefined role (no formal role)",
      },
      {
        role: "",
        expectedBadge: true,
        description: "empty string role (no formal role)",
      },
      {
        role: "GUEST",
        expectedBadge: true,
        description: "GUEST role (non-formal role)",
      },
      {
        role: "VIEWER",
        expectedBadge: true,
        description: "VIEWER role (non-formal role)",
      },
      {
        role: "ANONYMOUS",
        expectedBadge: true,
        description: "ANONYMOUS role (non-formal role)",
      },
      {
        role: "invalid",
        expectedBadge: true,
        description: "invalid role (non-formal role)",
      },
    ];
  ...
  async testStatusBadgeVisibility() {
    ...
    this.recordTest({
      ...
      expected: shouldShowBadge
        ? "Badge elements visible"
        : "Badge elements hidden",
      actual: `Found ${badgeElements.length} badge elements`,
      passed: shouldShowBadge
  ...
  async testDropdownVisibility() {
    ...
    this.recordTest({
      ...
      input: `Current role: ${currentRole}`,
      expected: shouldHaveDropdown
        ? "Dropdown elements present"
        : "No dropdown requirements",
  ...
  async testDOMConsistency() {
    ...
    const visibleDropdowns = Array.from(dropdownElements).filter(
      (el) => el.offsetWidth > 0 && el.offsetHeight > 0,
    ).length;
  ...
  generateRecommendations() {
    ...
    if (failedTests.length === 0) {
      recommendations.push({
        ...
        description:
          "All logic tests passed. Ready for browser-based validation.",
      });
    } else {
