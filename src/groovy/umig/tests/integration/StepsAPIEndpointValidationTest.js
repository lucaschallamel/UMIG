...
describe('US-028 Steps API Endpoint Validation', function() {
  ...
  after(async function() {
    ...
    const reportPath = path.join(__dirname, '../reports/steps-api-validation-report.json');
    await fs.writeFile(reportPath, JSON.stringify({
      timestamp: new Date().toISOString(),
      testSuite: 'US-028 Steps API Endpoint Validation',
      summary: {
        totalTests: testResults.endpointTests.length + testResults.urlConstructionTests.length + 
                   testResults.comparisonTests.length + testResults.diagnosticTests.length,
  ...
  describe('1. Direct API Endpoint Tests', function() {
    ...
    it('should validate /rest/scriptrunner/latest/custom/steps endpoint exists', async function() {
      ...
      try {
        ...
        expect(response.status).to.not.equal(404, `Steps endpoint should exist at ${url}`);
        ...
        if (response.ok) {
          ...
          console.log('⚠ Steps endpoint exists but returned status:', response.status);
        }
    ...
    it('should validate /rest/scriptrunner/latest/custom/teams endpoint works (control test)', async function() {
      ...
      try {
        ...
        expect(response.status).to.not.equal(404, `Teams endpoint should exist at ${url}`);
        ...
        console.error('✗ Control test failed - Teams endpoint issue:', error.message);
        throw error;
      }
    });
    ...
    it('should test steps endpoint with iterationId parameter', async function() {
      ...
      try {
        ...
        if (response.status === 404) {
          console.log('✗ Steps endpoint with iterationId returns 404 - ROOT CAUSE IDENTIFIED');
          expect.fail(`Steps endpoint with iterationId returns 404: ${url}`);
        } else if (response.ok) {
          const data = await response.json();
          console.log('✓ Steps endpoint with iterationId works - returned', typeof data);
        } else {
          console.log('⚠ Steps endpoint with iterationId returned status:', response.status);
        }
    ...
    it('should test steps/master endpoint for dropdown data', async function() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('✓ Steps master endpoint works - returned', data.length, 'items');
        } else {
          console.log('✗ Steps master endpoint failed with status:', response.status);
        }
    ...
    it('should test alternative endpoints that might exist', async function() {
      ...
      for (const endpoint of alternativeEndpoints) {
        ...
        try {
          ...
          if (response.status !== 404) {
            console.log('ℹ Alternative endpoint found:', endpoint, 'status:', response.status);
          }
          
        } catch (error) {
          console.log('ℹ Alternative endpoint', endpoint, 'not accessible:', error.message);
        }
      }
    });
  });
  ...
  describe('2. URL Construction Validation', function() {
    ...
    it('should validate StepsAPIv2Client URL construction pattern', function() {
      ...
      expect(constructedUrl).to.equal(expectedUrl, 'URL construction should match expected pattern');
    ...
    it('should compare with working TeamsAPI URL pattern', function() {
      ...
      const result = {
        ...
        baseMatches: teamsUrl.replace('/teams', '') === stepsUrl.replace('/steps', ''),
  ...
  describe('4. Response Format Validation', function() {
    ...
    it('should validate Steps API response format matches expected structure', async function() {
      ...
      try {
        ...
        if (!response.ok) {
          console.log('⚠ Cannot test response format - endpoint not accessible');
          this.skip();
          return;
        }
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          result.hasExpectedFields = sample.hasOwnProperty('stm_id') && 
  ...
  describe('5. Error Diagnosis and Resolution', function() {
    ...
    it('should identify specific causes of 404 errors', async function() {
      ...
      for (const url of diagnosticUrls) {
        try {
          ...
          const result = {
            ...
            isControlEndpoint: url.includes('/teams') || url.includes('/migrations')
          };
          ...
          if (response.status === 404 && result.isStepsEndpoint) {
            ...
            console.log(response.status === 404 ? '✗' : '✓', 'Control endpoint -', url, '(', response.status, ')');
          }
      ...
      const controlEndpoints = diagnosisResults.filter(r => r.isControlEndpoint);
      
      const steps404Count = stepsEndpoints.filter(r => r.is404).length;
      const control404Count = controlEndpoints.filter(r => r.is404).length;
      
      console.log('\n=== DIAGNOSIS SUMMARY ===');
      console.log(`Steps endpoints with 404: ${steps404Count}/${stepsEndpoints.length}`);
      console.log(`Control endpoints with 404: ${control404Count}/${controlEndpoints.length}`);
      
      if (steps404Count > 0 && control404Count === 0) {
        console.log('🔍 ROOT CAUSE: Steps API not properly registered in ScriptRunner');
      } else if (steps404Count === 0) {
        console.log('✅ Steps API endpoints are accessible - issue may be elsewhere');
      } else {
        console.log('⚠ Mixed results - partial registration or configuration issue');
      }
    });
  });
});
...
function generateRecommendations(testResults) {
  ...
  const failedEndpoints = testResults.endpointTests.filter(t => t.status === 404 || t.error);
  if (failedEndpoints.length > 0) {
    recommendations.push({
      priority: 'HIGH',
      category: 'Endpoint Registration',
      issue: 'Steps API endpoints returning 404 errors',
      solution: 'Verify StepsApi.groovy is properly deployed and registered in ScriptRunner',
      actions: [
  ...
  const formatIssues = testResults.diagnosticTests.filter(t => t.hasExpectedFields === false);
