...
describe("StepView URL Construction Fix - Regression Prevention", () => {
  ...
  describe("Migrations API Response Format Validation", () => {
    test("should return migrations in correct array format", async () => {
      ...
      const problematicMigration = data.find(m => m.name === "Complex Migration with Spaces & Special-Chars");
      expect(problematicMigration).toBeDefined();
      
      const concatenatedMigration = data.find(m => m.name === "Operativebandwidth");
  ...
  describe("Iterations API Code Field Validation", () => {
    test("should include required code field for URL construction", async () => {
      ...
      const response = await fetch(`${BASE_URL}${SCRIPTRUNNER_BASE}/migrations/${testMigrationId}/iterations`);
      const data = await response.json();
      ...
      const complexIteration = data.find(i => i.name === "Iteration 1 for Plan Alpha-Beta");
      expect(complexIteration).toBeDefined();
      expect(complexIteration.code).toBe("IT1-ALPHA-BETA");
      
      const concatenatedIteration = data.find(i => i.name === "Iteration2forPlan");
    ...
    test("should handle missing migration gracefully", async () => {
      ...
      const response = await fetch(`${BASE_URL}${SCRIPTRUNNER_BASE}/migrations/${nonexistentId}/iterations`);
  ...
  describe("URL Construction Logic Validation", () => {
    test("should build correct URLs with proper encoding for complex names", () => {
      ...
      expect(migrationCode).toBe("Complex Migration with Spaces & Special-Chars");
      ...
      // Verify URL format
      expect(constructedUrl).toMatch(/^http:\/\/localhost:8090\/pages\/viewpage\.action\?/);
      expect(constructedUrl).toContain('pageId=1114120');
      expect(constructedUrl).toContain('mig=Complex+Migration+with+Spaces+%26+Special-Chars');
    ...
    test("should handle special characters and encoding correctly", () => {
      ...
      expect(url).not.toContain('&'); // Raw ampersand should be encoded
      expect(url).not.toContain('('); // Raw parentheses should be encoded
    });
    ...
    test("should use iteration code instead of name", () => {
      ...
      expect(iterationIdentifier).not.toBe("Very Long Iteration Name with Many Words");
    });
    ...
    test("should maintain URL format consistency", () => {
      ...
      expect(url).toMatch(/^http:\/\/localhost:8090\/pages\/viewpage\.action\?pageId=\d+&mig=.+&ite=.+&stepid=.+$/);
  ...
  describe("End-to-End URL Construction Workflow", () => {
    test("should complete full workflow from API data to valid URL", async () => {
      ...
      // Simulate API calls
      const migrationsResponse = await fetch(`${BASE_URL}${SCRIPTRUNNER_BASE}/migrations`);
      const migrations = await migrationsResponse.json();
      
      const iterationsResponse = await fetch(`${BASE_URL}${SCRIPTRUNNER_BASE}/migrations/mig-123/iterations`);
      ...
      expect(finalUrl).toMatch(/^http:\/\/localhost:8090\/pages\/viewpage\.action\?/);
      ...
      expect(mockFetch).toHaveBeenCalledWith(`${BASE_URL}${SCRIPTRUNNER_BASE}/migrations`);
      expect(mockFetch).toHaveBeenCalledWith(`${BASE_URL}${SCRIPTRUNNER_BASE}/migrations/mig-123/iterations`);
    });
  });
  ...
  describe("Performance and Reliability", () => {
    test("should handle large datasets without URL corruption", () => {
      ...
      const longMigrationName = "Very Long Migration Name That Could Potentially Cause Issues With URL Encoding And Special Characters Like & < > \" ' = ?";
      const longIterationCode = "VERY-LONG-ITERATION-CODE-2024-Q4-PROD-RELEASE-CANDIDATE-1"; 
      ...
      expect(url).toMatch(/^http:\/\/localhost:8090\/pages\/viewpage\.action\?/);
