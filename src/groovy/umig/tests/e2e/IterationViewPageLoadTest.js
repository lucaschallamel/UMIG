...
test.describe("US-028 Enhanced IterationView - Page Load UAT", () => {
  ...
  test("should load IterationView page and populate migration selector", async ({
    page,
  }) => {
    ...
    const firstMigrationOption = await migrationSelect
  ...
  test("should handle migration API network failures gracefully", async ({
    page,
  }) => {
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          select &&
          (select.textContent.includes("Error") ||
  ...
  test("should validate script loading order and DOM readiness", async ({
    page,
  }) => {
    ...
    const selectExists = events.find(
      (e) => e.event === "MigrationSelectExists",
    );
  ...
  test("should validate both migration loading functions execute correctly", async ({
    page,
  }) => {
    ...
    const migrationCalls = await page.evaluate(
      () => window.migrationLoadingCalls || [],
    );
  ...
  test("should complete full user workflow - migration selection to data display", async ({
    page,
  }) => {
    await page.goto(TEST_PAGE_URL);
    await page.waitForLoadState("networkidle");
  ...
  test("should meet performance requirements for page load", async ({
    page,
  }) => {
  ...
  test("should handle concurrent user scenarios", async ({ browser }) => {
    ...
    await Promise.all(pages.map((page) => page.goto(TEST_PAGE_URL)));
