...
class EmailDatabaseValidator {
  ...
  async validateEmailTemplatesTable() {
    ...
    try {
      ...
      this.logSuccess(
        `✓ Found ${activeResult.rows.length} active email template(s)`,
      );
      activeResult.rows.forEach((row) => {
        this.logInfo(
          `  → ${row.emt_type} (created: ${row.emt_created_date?.toISOString().split("T")[0] || "N/A"})`,
        );
      });
  ...
  async validateAuditLogTable() {
    ...
    try {
      ...
      if (emailAuditResult.rows.length > 0) {
        this.logSuccess(
          `✓ Found ${emailAuditResult.rows.length} email-related audit entry type(s)`,
        );
        emailAuditResult.rows.forEach((row) => {
          this.logInfo(
            `  → ${row.aud_action} (${row.aud_entity_type}): ${row.count} record(s)`,
          );
        });
      } else {
        this.logInfo(
          "  → No email-related audit entries found (this is normal for new installations)",
        );
      }
      ...
      this.logError(
        "✗ Error validating audit_log_aud for email auditing",
        error.message,
      );
      return false;
    }
  ...
  async validateSystemConfigurationTable() {
    ...
    try {
      ...
      this.logSuccess(
        `✓ Found ${configResult.rows.length} email/system configuration(s)`,
      );
      ...
      if (stepViewResult.rows.length > 0) {
        ...
        if (config.scf_is_active) {
          ...
          this.logWarning(
            "! StepView URL configuration exists but is inactive",
          );
        }
      } else {
      ...
      this.logError(
        "✗ Error validating system_configuration_scf",
        error.message,
      );
      return false;
    }
  ...
  async validateEmailInfrastructure() {
    ...
    try {
      ...
      if (fkResult.rows.length > 0) {
        this.logSuccess(
          `✓ Found ${fkResult.rows.length} email/audit-related foreign key relationship(s)`,
        );
        ...
        this.logInfo(
          "  → No email/audit-related foreign key relationships found",
        );
      }
