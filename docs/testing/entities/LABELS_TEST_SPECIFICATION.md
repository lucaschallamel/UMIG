...
describe("Labels Security Tests", () => {
  ...
  describe("Input Sanitization", () => {
    test("prevents XSS in label names and descriptions", async () => {
      const label = labelBuilder.withSpecialCharacters().build();
  ...
  describe("Search Security", () => {
    test("prevents search enumeration attacks", async () => {
      const searchAttempts = Array.from(
        { length: 100 },
        (_, i) => `system-label-${i}`,
      );

      const enumerationTest =
        await securityTester.testSearchEnumeration(searchAttempts);
    ...
    test("prevents information disclosure through search", async () => {
      ...
      const disclosureTest =
        await securityTester.testInformationDisclosure(sensitiveLabel);
  ...
  describe("Bulk Operation Security", () => {
    ...
    test("prevents bulk operation resource exhaustion", async () => {
      const largeBulkOperation = {
        action: "create",
        labels: Array.from({ length: 10000 }, () => labelBuilder.build()),
      };

      const resourceTest =
        await securityTester.testResourceExhaustion(largeBulkOperation);
...
describe("Labels Search Performance Tests", () => {
  ...
  test("faceted search performance", async () => {
    const facetedSearchBenchmark =
      await performanceTracker.measureFacetedSearch({
...
describe("Label Hierarchy and Relationships", () => {
  ...
  test("validates hierarchical constraints", async () => {
    const grandparent = labelBuilder.withCategory("root").build();
    ...
    const circularTest = await labelManager.validateHierarchy(
      child,
      grandparent,
    );
  ...
  test("manages label relationships", async () => {
    const urgentLabel = labelBuilder.withCategory("priority").build();
    ...
    const relationshipTest =
      await labelManager.validateRelationships(blockerLabel);
...
describe("Labels Integration Tests", () => {
  ...
  test("integrates with migration labeling", async () => {
    const priorityLabel = new LabelBuilder().asHighPriorityLabel().build();
  ...
  test("manages label lifecycle with entity relationships", async () => {
    const teamLabel = new LabelBuilder().withCategory("team").build();

    const team = new TeamBuilder().withLabels([teamLabel.labelId]).build();
...
beforeEach(() => {
  window.UMIGServices = {
    ...
    categoryService: {
      ...
      getCategories: jest
        .fn()
        .mockReturnValue(["general", "priority", "status"]),
