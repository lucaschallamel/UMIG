...
describe("Environments Security Tests", () => {
  ...
  describe("Configuration Security", () => {
    test("validates configuration injection attacks", async () => {
      ...
      const env = environmentBuilder.withConfiguration(maliciousConfig).build();
...
describe("Environment Configuration Validation", () => {
  ...
  test("validates resource limit boundaries", async () => {
    ...
    for (const testCase of testCases) {
      const env = environmentBuilder.withResourceLimits(testCase).build();
...
describe("Environments Performance Tests", () => {
  ...
  test("environment provisioning performance", async () => {
    const environments = Array.from({ length: 10 }, () =>
      new EnvironmentBuilder().asDevelopmentEnvironment().build(),
    );

    const benchmark =
      await performanceTracker.measureEnvironmentProvisioning(environments);
  ...
  test("configuration update performance", async () => {
    ...
    const updateBenchmark =
      await performanceTracker.measureConfigurationUpdates(
        env.environmentId,
        configUpdates,
      );
  ...
  test("environment health checking performance", async () => {
    ...
    const healthBenchmark =
      await performanceTracker.measureHealthChecks(environments);
...
describe("Environments Integration Tests", () => {
  ...
  test("manages environment lifecycle through migrations", async () => {
    const prodEnv = new EnvironmentBuilder().asProductionEnvironment().build();
...
beforeEach(() => {
  window.UMIGServices = {
    ...
    configurationService: {
      // CRITICAL for Environments entity
