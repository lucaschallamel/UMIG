...
describe('StepView Visual Alignment - HTML Structure', () => {
  test('doRenderStepDetails generates required CSS classes', () => {
    ...
    const requiredClasses = [
      'step-info', 'step-title', 'step-breadcrumb', 
      'breadcrumb-item', 'breadcrumb-separator',
      'step-key-info', 'step-metadata', 'metadata-item',
      'label', 'value', 'teams-container', 'team-section'
    ];
  ...
  test('metadata items follow exact IterationView pattern', () => {
    ...
    const metadataPattern = /<div class="metadata-item">.*?<span class="label">.*?<\/span>.*?<span class="value">.*?<\/span>.*?<\/div>/s;
  ...
  test('breadcrumb structure matches IterationView exactly', () => {
    ...
    const breadcrumbPattern = /<div class="step-breadcrumb">.*?<span class="breadcrumb-item">.*?<\/span>.*?<span class="breadcrumb-separator">›<\/span>/s;
...
describe('StepView Visual Alignment - CSS Classes', () => {
  test('status dropdown has correct classes and attributes', () => {
    const statusDropdown = /<select id="step-status-dropdown" class="status-dropdown pilot-only".*?>/;
    expect(generatedHTML).toMatch(statusDropdown);
  });

  test('instruction table uses IterationView column structure', () => {
    const columnClasses = ['col-num', 'col-instruction', 'col-team', 'col-control', 'col-duration', 'col-complete'];
  ...
  test('label tags include proper styling attributes', () => {
    ...
    const labelPattern = /<span class="label-tag" style="background-color:.*?color:.*?">/;
...
export function compareWithIterationView(standaloneHTML, iterationViewHTML) {
  ...
  return {
    structureMatch: JSON.stringify(standaloneStructure) === JSON.stringify(iterationStructure),
    differences: findStructureDifferences(standaloneStructure, iterationStructure)
  };
}
```
...
*Phase 4 Complete: Automated Test Suite ensures ongoing 100% visual alignment between IterationView and standalone StepView*