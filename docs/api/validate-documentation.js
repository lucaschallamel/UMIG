        ...
        Object.entries(spec.paths || {}).forEach(([endpoint, pathObj]) => {
            Object.entries(pathObj).forEach(([method, methodObj]) => {
                if (methodObj.requestBody?.content) {
                    Object.entries(methodObj.requestBody.content).forEach(([contentType, contentObj]) => {
                        if (contentObj.examples) {
                            Object.entries(contentObj.examples).forEach(([exampleName, example]) => {
                                examples.push({
                                    endpoint,
                                    ...
                                    expectedStatus: methodObj.responses ? 
                                        Object.keys(methodObj.responses)[0] : '200'
                                });
                            });
                        }
                    });
                ...
                if (methodObj.responses) {
                    Object.entries(methodObj.responses).forEach(([statusCode, responseObj]) => {
                        if (responseObj.content) {
                            Object.entries(responseObj.content).forEach(([contentType, contentObj]) => {
                                if (contentObj.examples) {
                                    Object.entries(contentObj.examples).forEach(([exampleName, example]) => {
                                        examples.push({
                                            endpoint,
...
class EndpointValidator {
    ...
    async validateEndpoint(endpoint, method) {
        ...
        try {
            ...
            this.tracker.addTest(endpoint, method, 'Endpoint Availability', 'passed', {
            ...
            this.tracker.addTest(endpoint, method, 'Endpoint Availability', 'failed', {
                error: error.message
            });
    ...
    async validateExample(example) {
        ...
        try {
            ...
            this.tracker.addTest(endpoint, method, 'Example Validation', 
    ...
    async validateAuthentication(endpoint, method) {
        try {
            ...
            if (unauthResponse.status === 401 || invalidAuthResponse.status === 401) {
                this.tracker.addTest(endpoint, method, 'Authentication Required', 'passed', {
                    unauthStatus: unauthResponse.status,
                    invalidAuthStatus: invalidAuthResponse.status,
                    validAuthStatus: validAuthResponse.status
                });
            } else {
                this.tracker.addTest(endpoint, method, 'Authentication Required', 'warning', {
            ...
            this.tracker.addTest(endpoint, method, 'Authentication Validation', 'failed', {
                error: error.message
            });
        }
    }
...
class DocumentationValidator {
    ...
    generateReport(results) {
        ...
        console.log(`⏱️  Duration: ${new Date(results.summary.endTime) - new Date(results.summary.startTime)}ms`);
