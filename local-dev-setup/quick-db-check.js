...
async function checkEmailTables() {
    ...
    try {
        ...
        tableCheck.rows.forEach(row => {
            ...
            tablesByName[row.table_name].push(`${row.column_name} (${row.data_type})`);
        });
        ...
        const templateCount = await client.query('SELECT COUNT(*) FROM email_templates_emt;');
        console.log(chalk.yellow(`📊 Email templates count: ${templateCount.rows[0].count}`));
        ...
        if (templateCount.rows[0].count > 0) {
            ...
            templates.rows.forEach(t => {
                console.log(chalk.gray(`  • ${t.emt_template_name}: ${t.emt_subject_line}`));
            });
        }
