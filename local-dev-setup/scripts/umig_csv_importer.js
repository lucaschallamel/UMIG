...
program
  ...
  .requiredOption(
    "--mapping-file <file>",
    "Path to JSON mapping file (db_field:csv_header)",
  )
  .option("--env <env>", 'Environment (must be "dev" or "test")', "dev")
  .option(
    "--reset",
    "Truncate all data in the table before import (interactive confirmation)",
  )
  .option(
    "--dry-run",
    "Validate input and show what would be imported, but do not modify the database",
  )
  .parse(process.argv);
...
if (!["dev", "test"].includes(options.env)) {
  console.error(
    "Refusing to run: This tool is only for dev/test environments.",
  );
  process.exit(1);
}
...
async function confirmReset(table) {
  return new Promise((resolve) => {
    ...
    rl.question(
      `\nWARNING: This will DELETE ALL DATA from the ${table} table. Are you sure you want to continue? (yes/no): `,
      (answer) => {
        rl.close();
          ...
          if (dryRun) {
            ...
            if (errors.length > 0) {
              ...
              errors.slice(0, 5).forEach((e) => {
                console.log(
                  `Row ${e.row}: Missing/Invalid fields: ${e.missing.join(", ")}`,
                );
              });
            }
          } else {
            console.log(
              `Import complete. Rows processed: ${rows.length}, valid: ${validRows}, invalid/skipped: ${invalidRows}`,
            );
            if (errors.length > 0) {
              console.log(`\nFirst 5 errors:`);
              errors.slice(0, 5).forEach((e) => {
