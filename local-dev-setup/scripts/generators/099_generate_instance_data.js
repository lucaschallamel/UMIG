...
async function generateInstanceData(config, options = {}) {
  ...
  for (const iteration of iterations.rows) {
    ...
    if (!masterPlanId) {
      console.warn(
        `  - Skipping iteration ${iteration.ite_id} as it has no linked master plan.`,
      );
      continue;
    }
    ...
    console.log(
      `Creating plan instance for iteration ${iteration.ite_id} using master plan ${masterPlanId}`,
    );
    ...
    console.log(
      `  Found ${masterSequences.rows.length} master sequences for plan ${masterPlanId}`,
    );

    for (const masterSequence of masterSequences.rows) {
      // 2. Create Sequence Instance (sqi)
      console.log(
        `    Creating sequence instance for master sequence ${masterSequence.sqm_id}`,
      );

      // Prepare override values for sequence instance
      const sqi_name = shouldOverrideAttribute()
        ? generateOverrideValue("name")
        : masterSequence.sqm_name;
      const sqi_description = shouldOverrideAttribute()
        ? generateOverrideValue("description")
        : masterSequence.sqm_description;
      const sqi_order = shouldOverrideAttribute()
        ? generateOverrideValue("order", masterSequence.sqm_order)
        : masterSequence.sqm_order;
      ...
      console.log(
        `      Found ${masterPhases.rows.length} master phases for sequence ${masterSequence.sqm_id}`,
      );

      for (const masterPhase of masterPhases.rows) {
        // 3. Create Phase Instance (phi)
        console.log(
          `        Creating phase instance for master phase ${masterPhase.phm_id}`,
        );

        // Prepare override values for phase instance
        const phi_name = shouldOverrideAttribute()
          ? generateOverrideValue("name")
          : masterPhase.phm_name;
        const phi_description = shouldOverrideAttribute()
          ? generateOverrideValue("description")
          : masterPhase.phm_description;
        const phi_order = shouldOverrideAttribute()
          ? generateOverrideValue("order", masterPhase.phm_order)
          : masterPhase.phm_order;
        ...
        console.log(
          `          Found ${masterSteps.rows.length} master steps for phase ${masterPhase.phm_id}`,
        );

        for (const masterStep of masterSteps.rows) {
          // 4. Create Step Instance (sti)
          console.log(
            `            Creating step instance for master step ${masterStep.stm_id}`,
          );

          // Prepare override values for step instance
          const sti_name = shouldOverrideAttribute()
            ? generateOverrideValue("name")
            : masterStep.stm_name;
          const sti_description = shouldOverrideAttribute()
            ? generateOverrideValue("description")
            : masterStep.stm_description;
          const sti_duration_minutes = shouldOverrideAttribute()
            ? generateOverrideValue("duration")
            : masterStep.stm_duration_minutes;
          ...
          console.log(
            `              Found ${masterInstructions.rows.length} master instructions for step ${masterStep.stm_id}`,
          );

          for (const masterInstruction of masterInstructions.rows) {
            // Prepare override values for instruction instance
            const ini_order = shouldOverrideAttribute()
              ? generateOverrideValue("order", masterInstruction.inm_order)
              : masterInstruction.inm_order;
            const ini_body = shouldOverrideAttribute()
              ? generateOverrideValue("description")
              : masterInstruction.inm_body;
            const ini_duration_minutes = shouldOverrideAttribute()
              ? generateOverrideValue("duration")
              : masterInstruction.inm_duration_minutes;
            ...
            try {
              ...
              console.error(
                `                ❌ Failed to create instruction instance: ${error.message}`,
              );
              throw error;
            }
          }
        ...
        console.log(
          `          Found ${masterControls.rows.length} master controls for phase ${masterPhase.phm_id}`,
        );
        ...
        for (const masterControl of masterControls.rows) {
          ...
          const cti_order = shouldOverrideAttribute()
            ? generateOverrideValue("order", masterControl.ctm_order)
            : masterControl.ctm_order;
          const cti_name = shouldOverrideAttribute()
            ? generateOverrideValue("name")
            : masterControl.ctm_name;
          const cti_description = shouldOverrideAttribute()
            ? generateOverrideValue("description")
            : masterControl.ctm_description;
          const cti_type = shouldOverrideAttribute()
            ? generateOverrideValue("type")
            : masterControl.ctm_type;
          const cti_is_critical = shouldOverrideAttribute()
            ? generateOverrideValue("boolean")
            : masterControl.ctm_is_critical;
          const cti_code = shouldOverrideAttribute()
            ? generateOverrideValue("code")
            : masterControl.ctm_code;
