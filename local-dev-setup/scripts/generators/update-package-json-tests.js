...
class PackageJsonUpdater {
  ...
  generateChangesSummary(original, updated) {
    ...
    console.log('   npm run test:integration    - Database/API integration tests');
    console.log('   npm run test:integration:watch - Integration tests in watch mode');
    console.log('');
    ...
    console.log('   npm test                    - Smart runner (detects infrastructure)');
    console.log('   npm run test:smart          - Same as above');
    console.log('   npm run test:quick          - Unit + DOM only (no infrastructure)');
    console.log('   npm run test:all            - All tests (requires infrastructure)');
    console.log('');
    
    console.log('💡 INFRASTRUCTURE DETECTION:');
    console.log('   - Smart runner checks for database, Confluence, and MailHog');
    console.log('   - Skips tests gracefully when infrastructure unavailable');
    console.log('   - Provides clear feedback about what\'s running and what\'s skipped');
    console.log('');
    
    console.log('🔧 EXPECTED OUTCOMES:');
    console.log('   ✅ Unit tests: 26/26 files passing (no infrastructure needed)');
    console.log('   ✅ DOM tests: 1/1 file passing (JSDOM environment fix)');
    console.log('   ⏭️  Integration tests: Skip gracefully without infrastructure');
...
`;
    ...
    const instructionsPath = path.resolve(__dirname, '../../TEST_SUITE_MIGRATION_INSTRUCTIONS.md');
