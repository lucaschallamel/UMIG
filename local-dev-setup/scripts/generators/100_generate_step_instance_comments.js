...
async function eraseStepInstanceCommentsTable(dbClient) {
  ...
  try {
    await dbClient.query('TRUNCATE TABLE step_instance_comments_sic RESTART IDENTITY CASCADE');
...
async function generateStepInstanceComments(config, options = {}) {
  ...
  const instances = await dbClient.query('SELECT sti_id FROM steps_instance_sti');
  const users = await dbClient.query('SELECT usr_id FROM users_usr');

  if (instances.rows.length === 0 || users.rows.length === 0) {
    console.error('Cannot generate instance comments without step instances and users.');
    return;
  }
