...
async function generateCanonicalPlans(config, options = {}) {
  ...
  const envRoles = await dbClient.query(
    "SELECT enr_id FROM environment_roles_enr",
  );
  const iterationTypes = await dbClient.query(
    "SELECT itt_code FROM iteration_types_itt",
  );

  // Fetch available statuses from status_sts table
  const planStatusesResult = await dbClient.query(
  ...
  if (
    teams.rows.length === 0 ||
    stepTypes.rows.length === 0 ||
    envRoles.rows.length === 0 ||
    iterationTypes.rows.length === 0 ||
  ...
  for (
    let i = 0;
    i < config.CANONICAL_PLANS.PER_MIGRATION * config.MIGRATIONS.COUNT;
    i++
  ) {
    ...
    for (let j = 0; j < numSequences; j++) {
      const seqRes = await dbClient.query(
        ...
        [
          plmId,
          j + 1,
          `Sequence ${j + 1}: ${faker.lorem.words(3)}`,
          faker.lorem.sentence(),
          "generator",
          new Date(),
          "generator",
      ...
      for (let k = 0; k < numPhases; k++) {
        ...
        for (let l = 0; l < numSteps; l++) {
          ...
          const targetEnvRoleId = faker.helpers.arrayElement(
            envRoles.rows,
          ).enr_id;
          ...
          if (teams.rows.length > 0) {
            const impactedTeams = faker.helpers.arrayElements(
              teams.rows,
              numImpactedTeams,
            );
          ...
          if (iterationTypes.rows.length > 0) {
            const iterationTypesToLink = faker.helpers.arrayElements(
              iterationTypes.rows,
              numIterationTypes,
            );
