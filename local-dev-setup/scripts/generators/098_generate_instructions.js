...
async function eraseInstructionsTables(dbClient) {
  console.log(
    "Erasing instructions_instance_ini and instructions_master_inm tables...",
  );
  try {
...
async function generateInstructions(config, options = {}) {
  ...
  const controlsRes = await dbClient.query(
  ...
  for (const { stm_id } of stepsRes.rows) {
    const numInstructions = faker.number.int({
      min: instrCfg.PER_STEP.MIN,
      max: instrCfg.PER_STEP.MAX,
    });
    for (let order = 1; order <= numInstructions; order++) {
      ...
      const ctm_id =
        instrCfg.OPTIONAL_CONTROL &&
        controlsRes.rows.length &&
        faker.datatype.boolean()
          ? faker.helpers.arrayElement(controlsRes.rows).ctm_id
          : null;
      // Random description
      const inm_body =
        instrCfg.BODY_TYPE === "sentence"
          ? faker.lorem.sentence()
          : faker.lorem.paragraph();
      // Random duration
      const inm_duration_minutes = faker.number.int({
        min: instrCfg.DURATION_MIN,
        max: instrCfg.DURATION_MAX,
      });
      instructionsMasters.push({
        stm_id,
        tms_id,
        ctm_id,
        inm_order: order,
