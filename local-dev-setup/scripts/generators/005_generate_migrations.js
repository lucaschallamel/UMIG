...
import { faker, randomDateInRange, nextThursday } from "../lib/utils.js";
...
async function generateMigrations(config, options = {}) {
  ...
  if (plansResult.rows.length < requiredPlans) {
    throw new Error(
      `Cannot generate migrations: Not enough canonical plans available. Need ${requiredPlans}, found ${plansResult.rows.length}.`,
    );
  }
  const plans = [...plansResult.rows]; // Create a mutable copy

  // Fetch available statuses from status_sts table
  const migrationStatusesResult = await client.query(
    "SELECT sts_id, sts_name FROM status_sts WHERE sts_type = $1",
    ["Migration"],
  );
  const migrationStatusIds = migrationStatusesResult.rows.map(
    (row) => row.sts_id,
  );

  const iterationStatusesResult = await client.query(
    "SELECT sts_id, sts_name FROM status_sts WHERE sts_type = $1",
    ["Iteration"],
  );
  const iterationStatusIds = iterationStatusesResult.rows.map(
    (row) => row.sts_id,
  );
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    for (const plan of assignedPlans) {
      await generateIterationsForPlan(
        migId,
        plan.plm_id,
        ITERATIONS,
        startDate,
...
async function generateIterationsForPlan(
  migId,
  planId,
  iterationConfig,
  migStartDate,
  ...
  console.log(
    `  - Generating iterations for plan ${planId} in migration ${migId}`,
  );
  for (const type in iterationConfig) {
    const config = iterationConfig[type];
    const count =
      typeof config === "number" ? config : faker.number.int(config);
