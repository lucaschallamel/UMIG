...
class MetadataGenerator {
  ...
  generateDeploymentManifest(buildInfo, versionInfo, environment = "dev") {
    ...
    const archiveSize =
      buildInfo.artifactPaths && buildInfo.artifactPaths.length > 0
    ...
    const deploymentManifest = {
      ...
      contents: {
        umig: {
          ...
          description:
            "Core ScriptRunner application with admin GUI components",
        },
        database: {
          files: "36+",
      ...
      deployment: {
        ...
        archiveSize:
          archiveSize > 0 ? this.formatBytes(archiveSize) : "unknown",
      },
      buildInfo: {
...
`;
    ...
    try {
      ...
      if (fs.existsSync(buildConfigPath)) {
        ...
        this.metadata.project.buildConfig = {
          ...
          environments: buildConfig.environments
            ? Object.keys(buildConfig.environments)
            : [],
        };
      }
    } catch (error) {
  ...
  collectEnvironmentMetadata() {
    ...
    this.metadata.environment.variables =
      this.getFilteredEnvironmentVariables();
  }
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        this.metadata.dependencies.counts = {
          ...
          development: Object.keys(this.metadata.dependencies.development)
            .length,
          total: Object.keys({
  ...
  collectComponentMetadata() {
    try {
      ...
      this.metadata.components.details.sort(
  ...
  scanComponentDirectory(dirPath, relativePath) {
    ...
    for (const file of files) {
      ...
      if (file.isDirectory()) {
        ...
        if (
          !this.metadata.components.summary.lastModified ||
          new Date(stats.mtime) >
            new Date(this.metadata.components.summary.lastModified)
        ) {
          this.metadata.components.summary.lastModified =
