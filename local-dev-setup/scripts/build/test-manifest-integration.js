...
async function testManifestIntegration() {
  ...
  try {
    ...
    const buildState = {
      ...
      endTime: new Date(),
      artifactPaths: ['/Users/lucaschallamel/Documents/GitHub/UMIG/build/umig-deployment-dev-2025-09-25T14-32-01.tar.gz'],
      errors: [],
      warnings: []
    };
    ...
    console.log(`   - Environment: ${deploymentManifest.packageInfo.environment}`);
    console.log(`   - Archive Size: ${deploymentManifest.deployment.archiveSize}`);

    // Test README generation
    console.log('\n4. Generating deployment README...');
    const deploymentReadme = metadataGenerator.generateDeploymentReadme(versionInfo, 'dev');
    ...
    console.log(`   OLD: build-manifest.json (${fs.statSync('/Users/lucaschallamel/Documents/GitHub/UMIG/build/build-manifest.json').size} bytes)`);
    console.log(`   NEW: ${manifestResult.filename} (${manifestResult.size} bytes)`);

    const reductionPercentage = ((fs.statSync('/Users/lucaschallamel/Documents/GitHub/UMIG/build/build-manifest.json').size - manifestResult.size) / fs.statSync('/Users/lucaschallamel/Documents/GitHub/UMIG/build/build-manifest.json').size * 100).toFixed(1);
