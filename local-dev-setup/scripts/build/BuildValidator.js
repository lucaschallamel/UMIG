...
class BuildValidator {
    ...
    async runPreValidation() {
        ...
        if (hasErrors) {
            console.error('❌ Pre-build validation failed:', this.validationResults.errors.length, 'errors');
            this.displayErrors();
        } else {
    ...
    async runPostValidation(buildOutputPath) {
        ...
        const hasErrors = this.validationResults.errors.filter(e => e.phase === 'postValidation').length > 0;
        if (hasErrors) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                try {
                    ...
                    this.addWarning('tar command not available - archive creation may fail');
                }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                try {
                    ...
                    if (status.trim()) {
                        this.addWarning('Uncommitted changes detected - build may not reflect current state');
                    }
                } catch (error) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!fs.existsSync(packageLockPath)) {
                    this.addWarning('package-lock.json not found - dependency versions may vary');
                }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (missingDirs.length > 0) {
                    throw new Error(`Required directories missing: ${missingDirs.join(', ')}`);
                }
    ...
    validateConfiguration() {
        return new Promise((resolve, reject) => {
            try {
                const configPath = path.join(this.projectRoot, 'local-dev-setup/build-config.json');
                ...
                const requiredSections = ['source', 'build', 'sources', 'environments', 'output'];
                ...
                if (config.source) {
                    const requiredSourceFields = ['baseDirectory', 'validation', 'deploymentStructure'];
                    for (const field of requiredSourceFields) {
                        if (!config.source[field]) {
                            throw new Error(`Missing required source configuration field: ${field}`);
                        }
                    }
                    ...
                    if (deploymentStructure) {
                        ...
                        for (const section of requiredSections) {
                            if (!deploymentStructure[section]) {
                                throw new Error(`Missing required deployment structure section: ${section}`);
                            }
                            if (!deploymentStructure[section].inclusionPatterns) {
                                throw new Error(`Missing inclusionPatterns for deployment section: ${section}`);
                            }
                        }
                    }
                }
                ...
                if (config.sources && Array.isArray(config.sources)) {
                    for (const source of config.sources) {
                        ...
                        if (source.required && !fs.existsSync(sourcePath)) {
                            this.addWarning(`Required source directory does not exist: ${source.path}`);
                        }
                    }
                }

                // Validate required directories from source validation
                if (config.source.validation && config.source.validation.requiredDirectories) {
    ...
    validateDiskSpace() {
        return new Promise((resolve, reject) => {
            try {
                ...
                reject({ check: 'diskSpace', error: 'Insufficient disk space or write permissions' });
            }
        });
    }
    ...
    validatePermissions() {
        return new Promise((resolve, reject) => {
            try {
                ...
                reject({ check: 'permissions', error: 'Insufficient file permissions' });
            }
        });
    }
    ...
    validateArchiveIntegrity(archivePath) {
        return new Promise((resolve, reject) => {
            try {
                ...
                reject({ check: 'archiveIntegrity', error: 'Archive integrity check failed' });
            }
        });
    }
    ...
    validateFilePermissions(buildOutputPath) {
        return new Promise((resolve) => {
            ...
            try {
                ...
                resolve({ check: 'filePermissions', status: 'warning', message: 'File permissions may be incorrect' });
            }
        });
    }
    ...
    validateManifestFiles(buildOutputPath) {
        return new Promise((resolve) => {
            ...
            const manifestPath = buildOutputPath.replace(/\.(tar\.gz|zip)$/, '-manifest.json');
            if (fs.existsSync(manifestPath)) {
                try {
                    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
                    resolve({ check: 'manifestFiles', status: 'passed', manifest });
                } catch (error) {
                    resolve({ check: 'manifestFiles', status: 'warning', message: 'Manifest file is invalid JSON' });
                }
            } else {
                resolve({ check: 'manifestFiles', status: 'warning', message: 'No manifest file found' });
            }
        });
    }
    ...
    getSummary() {
        ...
        const postChecks = Object.keys(this.validationResults.postValidation).length;
    ...
    async validateFileSystemPermissions() {
        return new Promise((resolve, reject) => {
            try {
                ...
                reject({ check: 'fileSystemPermissions', error: 'Insufficient file permissions' });
            }
        });
    }
    ...
    async validateArtifacts(artifactPaths) {
        return new Promise((resolve, reject) => {
            try {
                ...
                console.log(`✓ Artifact validation passed: ${artifactPaths.length} artifacts`);
                resolve({ check: 'artifacts', status: 'passed', count: artifactPaths.length });
            } catch (error) {
    ...
    async validateArchiveIntegrity(artifactPaths) {
        return new Promise((resolve, reject) => {
            try {
                const archiveFiles = artifactPaths.filter(path => path.endsWith('.tar.gz') || path.endsWith('.zip'));

                if (archiveFiles.length === 0) {
                    resolve({ check: 'archiveIntegrity', status: 'skipped', message: 'No archive files to validate' });
                    return;
                }
                ...
                console.log(`✓ Archive integrity validation passed: ${archiveFiles.length} archives`);
                resolve({ check: 'archiveIntegrity', status: 'passed', count: archiveFiles.length });
            } catch (error) {
                reject({ check: 'archiveIntegrity', error: 'Archive integrity check failed: ' + error.message });
            }
        });
    }
    ...
    async validateMetadataConsistency(artifactPaths) {
        return new Promise((resolve) => {
            try {
                ...
                const manifestFiles = [];
                for (const artifactPath of artifactPaths) {
                    const manifestPath = artifactPath.replace(/\.(tar\.gz|zip)$/, '-manifest.json');
                    if (fs.existsSync(manifestPath)) {
                        manifestFiles.push(manifestPath);
                    }
                }

                console.log(`✓ Metadata consistency validation passed: ${manifestFiles.length} manifest files`);
                resolve({ check: 'metadataConsistency', status: 'passed', manifestCount: manifestFiles.length });
            } catch (error) {
                resolve({ check: 'metadataConsistency', status: 'warning', message: error.message });
            }
        });
    }
    ...
    async validateEnvironmentCompatibility() {
        return new Promise((resolve) => {
            try {
                ...
                if (majorVersion < 14) {
                    throw new Error(`Node.js ${nodeVersion} may not be compatible. Requires Node.js 14+`);
                }

                console.log('✓ Environment compatibility validation passed');
                resolve({ check: 'environmentCompatibility', status: 'passed', nodeVersion });
            } catch (error) {
                resolve({ check: 'environmentCompatibility', status: 'warning', message: error.message });
            }
        });
    }
