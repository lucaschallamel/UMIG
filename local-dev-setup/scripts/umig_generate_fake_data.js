...
async function main() {
  ...
  program
    ...
    .option(
      "--erase",
      "Erase the relevant database tables before generating data",
    )
    .option(
      "--script <number>",
      "Run only a specific generator script (e.g., 01, 02, etc.)",
    )
    .parse(process.argv);

  const options = program.opts();
  // Detect --<3-digit> generator flags (e.g., --099)
  const argGenerators = process.argv
    .filter((arg) => /^--\d{3}$/.test(arg))
    .map((arg) => arg.slice(2));

  const generators = {
    "001": () => generateCoreMetadata(), // No reset needed
    "002": () =>
      generateTeamsAndApps(CONFIG, { ...options, clientOverride: client }),
    "003": () => generateUsers(CONFIG, { ...options, clientOverride: client }),
    "004": () =>
      generateCanonicalPlans(CONFIG, { ...options, clientOverride: client }),
    // Insert pilot comments after canonical plans
    "009": () =>
      generateStepPilotComments(CONFIG, { ...options, clientOverride: client }),
    "005": () =>
      generateMigrations(CONFIG, { ...options, clientOverride: client }),
    "006": () =>
      generateAllEnvironments(CONFIG.ENVIRONMENTS, {
        ...options,
        clientOverride: client,
      }),
    "008": () => generateLabels(CONFIG, { ...options, clientOverride: client }),
    "007": () =>
      generateControls(CONFIG, { ...options, clientOverride: client }),
    "098": () =>
      generateInstructions(CONFIG, { ...options, clientOverride: client }),
    "099": () =>
      generateInstanceData(CONFIG, { ...options, clientOverride: client }),
    // Insert instance comments after all instance data
    100: () =>
      generateStepInstanceComments(CONFIG, {
        ...options,
        clientOverride: client,
      }),
  };
