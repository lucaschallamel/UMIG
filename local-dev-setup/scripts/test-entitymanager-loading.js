...
async function testEntityManagers() {
  ...
  try {
    ...
    await page.goto("http://localhost:8090/confluence/login.action", {
    ...
    if (moduleLoaderStatus.exists) {
      ...
      console.log(
        `  Loaded: ${moduleLoaderStatus.loadedModules.length} modules`,
      );
      console.log(
        `  Failed: ${moduleLoaderStatus.failedModules.length} modules`,
      );
      if (moduleLoaderStatus.failedModules.length > 0) {
        console.log(
          `  Failed modules: ${moduleLoaderStatus.failedModules.join(", ")}`,
        );
      }
    }
    ...
    for (const entity of ENTITY_MANAGERS) {
      ...
      if (isAvailable && isLoaded) {
        ...
        console.log(
          `  ⚠️  ${managerName}: Available but NOT loaded in adminGui`,
        );
        allPassed = false;
      } else {
    ...
    if (allPassed) {
      console.log(
        "✅ ALL TESTS PASSED - EntityManagers are working correctly!",
      );
    } else {
