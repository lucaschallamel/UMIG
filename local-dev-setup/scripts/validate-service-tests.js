...
function checkTestFile(serviceName) {
  ...
  const hasStandardSetup =
    content.includes("global.window") || content.includes("global.performance");
  ...
  if (!hasStandardSetup) {
    console.log(
      `⚠️  ${serviceName}: Missing global setup (window, performance mocks)`,
    );
    results.warnings.push({
      service: serviceName,
      issue: "Missing global setup",
    });
  }
  ...
  try {
    ...
    if (failed === 0 && passed > 0) {
      ...
      console.log(
        `⚠️  ${serviceName}: ${passed}/${total} tests passing (${passRate}%)`,
      );
      ...
      console.log(
        `❌ ${serviceName}: 0/${total} tests passing - All tests failing`,
      );
      return {
        service: serviceName,
        status: "all_failed",
...
if (results.total > 0) {
  ...
  console.log(
    `   Failing: ${results.total - results.passing} (${100 - overallPassRate}%)`,
  );
} else {
...
console.log(
  `   Services Partially Working: ${results.failed.filter((r) => r.status === "partial").length}/6`,
);
console.log(
  `   Services Not Working: ${results.failed.filter((r) => r.status !== "partial").length}/6`,
);
...
const needsPatternFix = results.failed.filter(
  (r) => r.status === "pattern_issue",
);
const needsTestFix = results.failed.filter(
  (r) => r.status === "partial" || r.status === "all_failed",
);
const needsExecution = results.failed.filter(
  (r) => r.status === "execution_failed",
);
...
if (needsPatternFix.length > 0) {
  ...
  needsPatternFix.forEach((r) => {
    console.log(
      `      - ${r.service}: ${r.issues ? r.issues.join(", ") : r.reason}`,
    );
  });
}
...
if (needsTestFix.length > 0) {
  ...
  needsTestFix.forEach((r) => {
    console.log(
      `      - ${r.service}: ${r.failed || 0} failing, ${r.passed || 0} passing`,
    );
  });
}
...
if (currentRate >= SUCCESS_THRESHOLD) {
  console.log(
    `🎉 SUCCESS: ${Math.round(currentRate * 100)}% pass rate achieved!`,
  );
  ...
  console.log(
    `   Need ${additional} more tests to pass to reach 90% threshold`,
  );
