...
const IMPORT_API_ENDPOINTS = [
  {
    ...
    testData: '{"test": "data", "migrations": [{"mig_name": "Test Migration", "mig_description": "Test description"}]}',
  ...
  },
  {
    ...
    testData: "tms_id,tms_name,tms_email,tms_description\\n1,Test Team,test@example.com,Test team description",
  ...
  },
  {
    ...
    method: "POST",
    contentType: "text/csv",
    testData: "usr_id,usr_code,usr_first_name,usr_last_name,usr_email,usr_is_admin,tms_id,rls_id\\n1,TST,Test,User,testuser@example.com,false,1,1",
  ...
  },
  {
    ...
    method: "POST",
    contentType: "application/json",
    testData: '{"plan": {"name": "Test Master Plan", "description": "Test master plan"}}',
...
async function testImportEndpoint(endpoint) {
  ...
  for (let attempt = 1; attempt <= CONFIG.retryAttempts; attempt++) {
    try {
      ...
      if (endpoint.method === "GET") {
        ...
        const contentTypeHeader = endpoint.contentType ? `-H "Content-Type: ${endpoint.contentType}"` : "";
        const dataParam = endpoint.testData ? `--data '${endpoint.testData}'` : "";
      ...
      if (statusCode >= 200 && statusCode < 300 && responseBody) {
        try {
          if (endpoint.path.includes("/template")) {
            ...
            contentTypeValid = responseBody.includes(",") || responseBody.includes("tms_") || responseBody.includes("usr_");
          } else if (endpoint.method === "GET" || endpoint.contentType === "application/json") {
          ...
          contentTypeValid = false;
          if (CONFIG.verbose) {
            printWarning(`Content validation failed for ${endpoint.name}: ${e.message}`);
          }
        }
      }
      ...
      if (statusValid) {
        ...
        printError(`${endpoint.name} - Status: ${statusCode} (expected: ${endpoint.expectedStatusRange.join(" or ")}) ✗`);
        testResult.error = `Unexpected status code: ${statusCode}`;
      }

      if (endpoint.performanceCritical) {
        printPerformance(`${endpoint.name} - Performance`, testResult.responseTime, CONFIG.performanceThreshold);
        if (!performanceValid) {
      ...
      if (CONFIG.verbose && responseBody && responseBody.length < 500) {
        console.log(colors.dim + `Response: ${responseBody.substring(0, 200)}${responseBody.length > 200 ? "..." : ""}` + colors.reset);
      }
      ...
      if (attempt === CONFIG.retryAttempts) {
        printError(`${endpoint.name} - Failed after ${CONFIG.retryAttempts} attempts: ${error.message}`);
      } else {
        printWarning(`${endpoint.name} - Attempt ${attempt} failed, retrying...`);
  ...
  if (testResult.success) {
    ...
    testResults.failed++;
    if (testResult.error?.includes("authentication") || testResult.error?.includes("unauthorized")) {
...
async function testContentTypeValidation() {
  ...
  const jsonEndpoint = IMPORT_API_ENDPOINTS.find(e => e.path === "/import/json");
  if (jsonEndpoint) {
    try {
      ...
      if (statusCode >= 400) {
        printSuccess(`JSON endpoint correctly rejected invalid content type (${statusCode})`);
      } else {
        printWarning(`JSON endpoint should reject invalid content type, but returned ${statusCode}`);
      }
    } catch (error) {
  ...
  const csvEndpoint = IMPORT_API_ENDPOINTS.find(e => e.path === "/import/csv/teams");
  if (csvEndpoint) {
    try {
      ...
      if (statusCode >= 400) {
        printSuccess(`CSV endpoint correctly rejected invalid content type (${statusCode})`);
      } else {
        printWarning(`CSV endpoint should reject invalid content type, but returned ${statusCode}`);
      }
    } catch (error) {
      printWarning(`Content type validation test failed: ${error.message}`);
    }
  ...
  for (const testCase of testCases) {
    try {
      ...
      if (testCase.expectedStatus.includes(statusCode)) {
        printSuccess(`${testCase.name} correctly handled malformed data (${statusCode})`);
      } else {
        printWarning(`${testCase.name} should reject malformed data, but returned ${statusCode}`);
      }
    } catch (error) {
...
async function testPerformanceUnderLoad() {
  ...
  const lightEndpoint = IMPORT_API_ENDPOINTS.find(e => e.path === "/import/status");
  if (!lightEndpoint) return;

  const concurrentRequests = 5;
  const promises = [];

  printInfo(`Testing ${concurrentRequests} concurrent requests to ${lightEndpoint.path}`);
  ...
  for (let i = 0; i < concurrentRequests; i++) {
    const promise = new Promise((resolve) => {
      ...
      try {
        ...
        resolve({ success: false, time: Date.now() - requestStart, error: error.message });
      }
    });
  ...
  try {
    ...
    if (successfulRequests === concurrentRequests) {
      ...
      printPerformance(`Average response time under load`, Math.round(averageTime), CONFIG.performanceThreshold);
    } else {
      printWarning(`Only ${successfulRequests}/${concurrentRequests} concurrent requests succeeded`);
    }
...
function generateTestReport() {
  ...
  console.log(`  Performance Failures: ${colors.yellow}${testResults.performanceFailures}${colors.reset}`);
  console.log(`  Content Type Failures: ${colors.yellow}${testResults.contentFailures}${colors.reset}`);
  console.log(`  Authentication Failures: ${colors.yellow}${testResults.authFailures}${colors.reset}`);
  ...
  if (testResults.failed > 0) {
    ...
    testResults.tests
      ...
      .forEach(test => {
        console.log(`  ❌ ${test.name}: ${test.error || `Status ${test.statusCode} (expected: success)`}`);
        if (test.responseTime > CONFIG.performanceThreshold) {
          console.log(`     Performance: ${test.responseTime}ms (exceeds ${CONFIG.performanceThreshold}ms threshold)`);
        }
      });
  }
  ...
  if (performanceTests.length > 0) {
    ...
    if (performancePass) {
      ...
      printWarning(`${testResults.performanceFailures} performance tests failed ⚠️`);
    }
  }
  ...
  console.log(`  API Coverage: ${coverage}% (${testResults.passed}/${testResults.total} endpoints)`);
  ...
  if (coverage >= 95) {
    ...
    printError(`Insufficient API coverage - ${coverage}% is below acceptable threshold ❌`);
  }
...
async function main() {
  try {
    ...
    printInfo("Testing all import API endpoints for functionality, performance, and compliance");
