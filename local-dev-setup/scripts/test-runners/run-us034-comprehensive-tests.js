...
function printBanner() {
  ...
  console.log(
    colorize("🚀 US-034 Data Import Comprehensive Test Suite", "bright"),
  );
  console.log(
    colorize("   Testing all import APIs, services, and workflows", "blue"),
  );
  console.log(colorize("=" * 80, "cyan"));
  console.log();
}
  ...
  const checks = [
    {
      ...
      check: async () => {
        try {
          ...
          return {
            success: true,
            message: `✅ Confluence running on ${CONFLUENCE_BASE_URL}`,
          };
        } catch (error) {
          return {
            success: false,
            message: `❌ Confluence not accessible: ${error.message}`,
          };
        }
      },
    },
    {
      ...
      check: async () => {
        try {
          ...
          return {
            success: false,
            message: `❌ PostgreSQL not ready: ${error.message}`,
          };
        }
      },
    },
...
async function runGroovyTest(testPath, description, timeout = TEST_TIMEOUT) {
  ...
  try {
    const result = execSync(`groovy -cp "src/groovy:lib/*" "${testPath}"`, {
      timeout: timeout,
      encoding: "utf8",
    ...
    console.log(
      colorize(`   ✅ ${description} - PASSED (${duration}ms)`, "green"),
    );
    ...
    const duration = Date.now() - startTime;
    console.log(
      colorize(`   ❌ ${description} - FAILED (${duration}ms)`, "red"),
    );
...
async function runUnitTests() {
  ...
  for (const test of unitTests) {
    ...
    if (fs.existsSync(test.path)) {
      ...
      console.log(
        colorize(
          `   ⚠️ ${test.description} - SKIPPED (file not found)`,
          "yellow",
        ),
      );
      results.push({
...
async function runIntegrationTests() {
  ...
  for (const test of integrationTests) {
    if (fs.existsSync(test.path)) {
      const result = await runGroovyTest(
        test.path,
        test.description,
        test.timeout,
      );
...
function analyzePerformanceMetrics(output, testName) {
  ...
  if (metrics.apiResponseTimes.length > 0) {
    ...
    console.log(
      `         API Calls: ${metrics.passedApiCalls + metrics.failedApiCalls}`,
    );
    console.log(`         Avg Response Time: ${Math.round(avgApiTime)}ms`);
    console.log(`         Max Response Time: ${maxApiTime}ms`);
    console.log(
      `         Performance Target Met: ${maxApiTime <= PERFORMANCE_THRESHOLD_MS ? "✅" : "❌"}`,
    );
  }
}
...
async function runCSVValidationTests() {
  ...
  console.log(
    "   🧪 Testing CSV template availability and import workflows...",
  );

  const csvTests = ["teams", "users", "applications", "environments"];
  ...
  for (const entityType of csvTests) {
    ...
    try {
      ...
      console.log(
        colorize(
          `   ✅ ${entityType} CSV template - VALIDATED (${duration}ms)`,
          "green",
        ),
      );
      ...
      console.log(
        colorize(
          `   ❌ ${entityType} CSV template - FAILED (${duration}ms)`,
          "red",
        ),
      );
...
async function runEndToEndTests() {
  ...
  for (const workflow of workflows) {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      console.log(
        colorize(`   ✅ ${workflow.name} - PASSED (${duration}ms)`, "green"),
      );
      ...
      console.log(
        colorize(
          `   ⚠️ ${workflow.name} - SKIPPED (command not available)`,
          "yellow",
        ),
      );
      ...
      results.push({
        ...
        output: error.stdout || "",
        error:
          "Command not available - using existing integration tests instead",
      });
    }
  }
...
async function generateReport() {
  ...
  const totalPassed = testResults.suiteResults.reduce(
    (sum, suite) => sum + suite.passed,
    0,
  );
  const totalTests = testResults.suiteResults.reduce(
  ...
  console.log(
    `   Test Suites: ${testResults.passedSuites}/${testResults.totalSuites} passed`,
  );
  console.log(`   Individual Tests: ${totalPassed}/${totalTests} passed`);
  console.log(`   Success Rate: ${successRate.toFixed(1)}%`);
  console.log(
    `   Total Duration: ${(testResults.totalDuration / 1000).toFixed(1)}s`,
  );

  // Production readiness assessment
  const criticalFailures = testResults.suiteResults
    .filter(
      (suite) =>
        suite.suiteName.includes("Integration") ||
        suite.suiteName.includes("Performance"),
    )
  ...
  if (testResults.productionReady) {
    ...
    testResults.suiteResults.forEach((suite) => {
      if (suite.passed < suite.total) {
        ...
        console.log(
          `   • ${suite.suiteName}: ${failureRate.toFixed(1)}% failure rate`,
        );
      }
    });
  }
  ...
  const reportPath =
    "src/groovy/umig/tests/integration/US034_COMPREHENSIVE_TEST_REPORT.md";
...
async function main() {
  ...
  try {
    ...
    testResults.passedSuites = testResults.suiteResults.filter(
      (suite) => suite.passed === suite.total,
    ).length;
    testResults.failedSuites =
      testResults.totalSuites - testResults.passedSuites;
    ...
    console.error(
      colorize(`💥 Test suite execution failed: ${error.message}`, "red"),
    );
    if (verbose) {
