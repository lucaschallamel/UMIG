...
const stopServices = async () => {
  log(yellow('Stopping and removing all services defined in podman-compose.yml...'));
...
const eraseUmigDatabase = async () => {
  ...
  try {
    const { stdout } = await execa('podman', ['inspect', '--format', '{{.State.Running}}', 'umig_postgres']);
    if (stdout.trim() !== 'true') {
  ...
  while (Date.now() - startTime < readyCheckTimeout) {
    try {
      await execa('podman', ['exec', 'umig_postgres', 'pg_isready', '-U', process.env.POSTGRES_USER]);
      isReady = true;
      break;
    } catch (error) {
  ...
  try {
    ...
    await execa('podman', ['exec', 'umig_postgres', 'psql', '-U', pgUser, '-d', 'postgres', '-c', terminateConnectionsSql]);
    ...
    log(yellow(`Warning: Could not terminate connections (this is expected if no connections exist).`));
  }

  log(yellow(`Dropping UMIG database (${dbName})...`));
  const dropDbSql = `DROP DATABASE IF EXISTS ${dbName};`;
  try {
    await execa('podman', ['exec', 'umig_postgres', 'psql', '-U', pgUser, '-d', 'postgres', '-c', dropDbSql]);
...
const eraseVolumes = async ({ eraseAll, eraseConfluence }) => {
  if (eraseAll) {
    ...
    try {
      ...
      log(yellow('UMIG database volume not found or could not be removed, skipping.'));
    }
  }
  if (eraseConfluence || eraseAll) {
    log(yellow('Erasing Confluence database and data volumes...'));
    try {
      await execa('podman', ['volume', 'rm', 'local-dev-setup_confluence_db_data']);
      log(green('Confluence database volume removed.'));
    } catch (error) {
      log(yellow('Confluence database volume not found or could not be removed, skipping.'));
    }
    try {
      await execa('podman', ['volume', 'rm', 'local-dev-setup_confluence_data']);
      log(green('Confluence data volume removed.'));
    } catch (error) {
      log(yellow('Confluence data volume not found or could not be removed, skipping.'));
    }
  }
};
...
export const stop = async () => {
  ...
  if (isErase && !eraseFlags.eraseUmig && !eraseFlags.eraseConfluence && !eraseFlags.eraseAll) {
    eraseFlags.eraseAll = true;
  }

  if (isErase) {
    let promptMessage = '❓ Are you sure you want to perform the following actions?\n';
