...
const checkPostgresHealth = async () => {
  try {
    const { stdout } = await execa('podman', ['inspect', '--format', '{{.State.Health.Status}}', 'umig_postgres']);
...
const ensureUmigDbExists = async () => {
  ...
  try {
    ...
    await execa('podman', ['exec', 'umig_postgres', 'psql', '-U', process.env.POSTGRES_USER, '-d', 'postgres', '-c', createUserSql]);
    log(green('UMIG user check/create complete.'));

    // 2. Create Database if it doesn't exist
    try {
      await execa('podman', ['exec', 'umig_postgres', 'psql', '-U', process.env.POSTGRES_USER, '-d', 'postgres', '-c', `CREATE DATABASE ${process.env.UMIG_DB_NAME}`]);
      log(green('UMIG database created.'));
    } catch (e) {
      if (e.stderr && e.stderr.includes(`database "${process.env.UMIG_DB_NAME}" already exists`)) {
    ...
    // 3. Grant Privileges
    await execa('podman', ['exec', 'umig_postgres', 'psql', '-U', process.env.POSTGRES_USER, '-d', 'postgres', '-c', grantPrivilegesSql]);
    log(green('UMIG user privileges granted.'));
  } catch (error) {
    log(red(`Failed to ensure UMIG database exists: ${error.stderr || error.message}`));
    throw error;
  }
};
