...
-- ==================================================
-- Update CHECK constraint to allow new template types
-- ==================================================

-- First, drop the existing check constraint (it will be recreated with expanded values)
ALTER TABLE email_templates_emt DROP CONSTRAINT IF EXISTS email_templates_emt_emt_type_check;

-- Add new check constraint with expanded allowed values, including any existing types
...
INSERT INTO email_templates_emt (
    ...
    true,
    'system'
) ON CONFLICT (emt_name) DO NOTHING;
