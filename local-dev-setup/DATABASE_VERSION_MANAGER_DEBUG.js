...
function analyzeComponentLoading() {
  ...
  try {
    ...
    console.log(
      `✅ DatabaseVersionManager class exists: ${analysis.classExists}`,
    );

    if (analysis.classExists) {
      analysis.details.classProperties = Object.getOwnPropertyNames(
        window.DatabaseVersionManager.prototype,
      );
      console.log(
        `📝 Class methods: ${analysis.details.classProperties.length} found`,
      );
      ...
      try {
        ...
        analysis.details.instanceMethods =
        ...
        keyMethods.forEach((method) => {
          analysis.details.keyMethods[method] =
        ...
        analysis.errors.push(
          `Instance creation failed: ${instanceError.message}`,
        );
        console.error(`❌ Instance creation failed:`, instanceError);
      }
    } else {
      analysis.errors.push(
        "DatabaseVersionManager class not found in window object",
      );
      console.error(
        "❌ DatabaseVersionManager class not found in window object",
      );
    ...
    if (window.componentRegistry) {
      analysis.details.registeredInAdmin =
        !!window.componentRegistry["database-version-manager"];
      console.log(
        `📋 Registered in admin GUI: ${analysis.details.registeredInAdmin}`,
      );
    }
    ...
    if (window.ComponentOrchestrator) {
      try {
        ...
        analysis.details.orchestratorIntegration = {
          exists: true,
          componentRegistered:
            orchestrator.registeredComponents?.has?.(
    ...
    analysis.status =
      analysis.classExists && analysis.instanceCreated ? "loaded" : "failed";
  } catch (error) {
...
function analyzeApiEndpoints() {
  ...
  const testEndpoints = [
    ...
    {
      name: "statistics",
      url: "/rest/scriptrunner/latest/custom/databaseVersions/statistics",
    },
    {
      name: "packageSQL",
      url: "/rest/scriptrunner/latest/custom/databaseVersions/packageSQL",
    },
    {
      name: "packageLiquibase",
      url: "/rest/scriptrunner/latest/custom/databaseVersions/packageLiquibase",
    },
    {
      name: "health",
      url: "/rest/scriptrunner/latest/custom/databaseVersions/health",
    },
  ];
...
function analyzeJavaScriptErrors() {
  ...
  console.log(
    "⚠️  Error capture activated - check browser dev tools console for actual errors",
  );
...
function analyzeDomStructure() {
  ...
  try {
    ...
    containerIds.forEach((id) => {
      ...
      console.log(
        `📦 Container ${id}: ${analysis.containers[id].exists ? "✅ Found" : "❌ Missing"}`,
      );
    });

    // Check for DatabaseVersionManager UI elements
    const dvmElements = document.querySelectorAll(
      '[class*="database-version"], [id*="database-version"], [data-component="database-version"]',
    );
    ...
    console.log(
      `🎯 DatabaseVersionManager elements: ${analysis.components.databaseVersionManager.elements} found`,
    );
    ...
    analysis.adminGui = {
      ...
      activeTab:
        document.querySelector(".nav-tabs .active")?.textContent || "unknown",
      menuItems: Array.from(document.querySelectorAll(".nav-tabs li")).map(
  ...
  try {
    ...
    const scripts = Array.from(
      document.querySelectorAll(
        'script[src*="DatabaseVersionManager"], script[src*="database"]',
      ),
    );
    ...
    console.log(
      `📜 JavaScript files: ${analysis.javascriptCache.scripts.length} found`,
    );

    // Check for cache-busting parameters
    const hasCacheBusting = scripts.some(
      (script) =>
        script.src.includes("?") ||
    ...
    const scriptRunnerMeta = document.querySelector(
      'meta[name="scriptrunner-version"]',
    );
    analysis.scriptRunnerCache = {
      version: scriptRunnerMeta?.content || "unknown",
      timestamp: new Date().toISOString(),
      cacheRecommendation:
        "Consider clearing ScriptRunner cache if issues persist",
    };

    console.log(
      `📋 ScriptRunner version: ${analysis.scriptRunnerCache.version}`,
    );
  } catch (error) {
...
function generateRecommendations(results) {
  ...
  if (
    results.componentLoading.classExists &&
    !results.componentLoading.instanceCreated
  ) {
    recommendations.push({
      priority: "HIGH",
  ...
  Object.entries(apiEndpoints).forEach(([name, endpoint]) => {
    if (endpoint.tested && !endpoint.ok) {
      recommendations.push({
        ...
        action:
          "Check API endpoint registration and ScriptRunner configuration",
        steps: [
  ...
  // Cache issues
  if (
    results.cacheAnalysis.javascriptCache &&
    !results.cacheAnalysis.javascriptCache.cacheBusting
  ) {
    recommendations.push({
      priority: "MEDIUM",
      category: "Caching",
...
function printSummaryReport(results) {
  ...
  console.log(
    `  🏗️ Instance creation: ${results.componentLoading.instanceCreated}`,
  );
  console.log("");
  ...
  console.log(
    "  1. Run: window.DatabaseVersionManager (check if class exists)",
  );
  console.log(
    '  2. Run: fetch("/rest/scriptrunner/latest/custom/databaseVersions") (test API)',
  );
  ...
  if (!results.componentLoading.classExists) {
    ...
    console.log(
      "  • API endpoints not responding - check backend registration",
    );
  }
...
function testComponentInstantiation() {
  ...
  try {
    ...
    methods.forEach((method) => {
      ...
      console.log(
        `${exists ? "✅" : "❌"} Method ${method}: ${exists ? "exists" : "missing"}`,
      );
    });
...
function testApiEndpoints() {
  ...
  endpoints.forEach(async (endpoint) => {
    try {
      ...
      console.log(
        `${icon} ${endpoint}: ${response.status} ${response.statusText}`,
      );
...
function checkScriptRunnerCache() {
  ...
  const dvmScripts = scripts.filter(
    (s) =>
...
console.log(
  "🎯 Additional tools: testComponentInstantiation(), testApiEndpoints(), checkScriptRunnerCache()",
);
