...
class MockGroovyTestRunner {
  ...
  executeGetUserActivityTest(scriptContent) {
    ...
    if (
      scriptContent.includes("DROP TABLE") ||
    ...
    if (
      scriptContent.includes("getUserActivity(1, -1)") ||
  ...
  executeAccessControlTest(scriptContent) {
    ...
    return results.length > 0
      ? results
      : { success: false, error: "No matching test" };
  }
  ...
  mockGetUserActivity(userId, days) {
    ...
    return [{ userId, action: "login", timestamp: new Date() }];
  }
