...
describe('TeamsEntityManager - Role Transition Management', () => {
  ...
  describe('validateRoleTransition', () => {
    test('should validate legitimate role transitions', () => {
      const result = teamsEntityManager.validateRoleTransition('USER', 'ADMIN', {
        role: 'SUPERADMIN'
      });
    ...
    test('should reject invalid role names', () => {
      const result = teamsEntityManager.validateRoleTransition('INVALID_ROLE', 'ADMIN', {
        role: 'SUPERADMIN'
      });
    ...
    test('should reject same role transitions', () => {
      const result = teamsEntityManager.validateRoleTransition('ADMIN', 'ADMIN', {
        role: 'SUPERADMIN'
      });
    ...
    test('should reject unauthorized transitions', () => {
      const result = teamsEntityManager.validateRoleTransition('USER', 'SUPERADMIN', {
        role: 'SUPERADMIN'
      });
    ...
    test('should prevent hierarchy violations', () => {
      ...
      expect(result.reason).toBe('Cannot assign a role equal to or higher than your own');
    });
  ...
  describe('changeUserRole', () => {
    ...
    test('should successfully change user role with full audit trail', async () => {
      ...
      expect(teamsEntityManager._checkPermission).toHaveBeenCalledWith('role_management');
    ...
    test('should handle permission cascade failure gracefully', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.result.cascadeWarning).toBe('Permission service unavailable');
    });

    test('should validate role transition before executing', async () => {
      // Mock invalid transition
      teamsEntityManager._getCurrentMemberRole.mockResolvedValue({ role: 'USER' });
  ...
  describe('cascadePermissions', () => {
    beforeEach(() => {
      ...
      teamsEntityManager._updateEntityPermissions = jest.fn().mockResolvedValue({
        success: true
      });

      teamsEntityManager._validateChildEntityPermissions = jest.fn().mockResolvedValue();
      teamsEntityManager._auditLog = jest.fn();
    });
    ...
    test('should cascade permissions to all related entities', async () => {
      ...
      expect(teamsEntityManager._updateEntityPermissions).toHaveBeenCalledTimes(3);
      ...
      expect(teamsEntityManager._validateChildEntityPermissions).toHaveBeenCalledWith(
        'team-123',
        'user-456',
        'ADMIN'
      );
    ...
    test('should handle cascade failure completely', async () => {
      ...
      teamsEntityManager._getUserRelatedEntities.mockRejectedValue(cascadeError);
  ...
  describe('getRoleHistory', () => {
    ...
    test('should retrieve role history from audit service', async () => {
      ...
      window.UMIGServices.auditService.getUserRoleHistory.mockResolvedValue(mockHistory);

      const result = await teamsEntityManager.getRoleHistory('user-456');

      expect(result).toEqual(mockHistory);
      expect(SecurityUtils.validateInput).toHaveBeenCalledWith({ userId: 'user-456' });
      expect(teamsEntityManager._checkPermission).toHaveBeenCalledWith('role_management');
      expect(window.UMIGServices.auditService.getUserRoleHistory).toHaveBeenCalledWith('user-456', 50);
    });
    ...
    test('should handle role history fetch failure', async () => {
      ...
      window.UMIGServices.auditService.getUserRoleHistory.mockRejectedValue(historyError);
  ...
  describe('Helper Methods', () => {
    describe('_canManageRole', () => {
      test('should allow SUPERADMIN to manage all roles', () => {
        expect(teamsEntityManager._canManageRole('SUPERADMIN', 'SUPERADMIN')).toBe(true);
        expect(teamsEntityManager._canManageRole('SUPERADMIN', 'ADMIN')).toBe(true);
        expect(teamsEntityManager._canManageRole('SUPERADMIN', 'USER')).toBe(true);
      });
      ...
      test('should allow users to manage roles below their level', () => {
        ...
        expect(teamsEntityManager._canManageRole('ADMIN', 'SUPERADMIN')).toBe(false);
      });
      ...
      test('should not allow USER to manage any roles', () => {
        ...
        expect(teamsEntityManager._canManageRole('USER', 'SUPERADMIN')).toBe(false);
      });

      test('should handle invalid roles gracefully', () => {
        expect(teamsEntityManager._canManageRole('INVALID', 'USER')).toBe(false);
        expect(teamsEntityManager._canManageRole('ADMIN', 'INVALID')).toBe(false);
      });
    });
    ...
    describe('_getCurrentMemberRole', () => {
      ...
      test('should fetch current member role from API', async () => {
        ...
        const result = await teamsEntityManager._getCurrentMemberRole('team-123', 'user-456');
  ...
  describe('Security and Integration', () => {
    test('should maintain enterprise security rating of 8.8/10+', () => {
      ...
      expect(teamsEntityManager.accessControls.SUPERADMIN).toContain('role_management');
