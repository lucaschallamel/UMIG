...
const { TestDatabaseManager } = require('../infrastructure/TestDatabaseManager');
const { MockGroovyTestRunner } = require('../infrastructure/MockGroovyTestRunner');
...
describe('User Security Tests', () => {
    ...
    describe('SQL Injection Prevention', () => {
        ...
        test('getUserActivity should validate userId parameter', async () => {
            ...
            const result = await groovyRunner.executeScript(testScript);
            expect(result.success).toBe(true);
            expect(result.message).toContain("User ID validation prevented injection");
        });
    ...
    describe('Authorization Controls', () => {
        test('canAccessUserActivity should enforce proper access controls', async () => {
            ...
            const otherUserRegular = results.find(r => r.test === "other_user_regular");
    ...
    describe('API Authorization Segregation', () => {
        ...
        test('POST cleanup endpoints should require admin privileges', async () => {
            ...
            const endpoint = '/rest/scriptrunner/latest/custom/users/cleanup-orphaned-members';
            const mockRequest = {
                method: 'POST',
                groups: ['confluence-administrators'],
                path: endpoint
