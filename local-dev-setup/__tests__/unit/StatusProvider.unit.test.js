...
const statusProviderPath = path.join(
  __dirname,
  "../../../src/groovy/umig/web/js/utils/StatusProvider.js",
);
...
describe("StatusProvider Unit Tests", () => {
  ...
  describe("Initialization and Configuration", () => {
    test("should initialize with correct configuration values", () => {
      ...
      expect(statusProvider.STATUS_ENDPOINT).toBe(
  ...
  describe("Core Functionality - getStatuses()", () => {
    test("should return cached data when available and valid", async () => {
      // Setup cache with valid data using MockStatusProvider
      const cachedData = [
        { name: mockStatusProvider.getStatusNameById(1), type: "Step", id: 1 },
      ];
      ...
      expect(consoleSpy.debug).toHaveBeenCalledWith(
        "StatusProvider: Cache hit for Step",
      );
    ...
    test("should fetch from API when cache is empty", async () => {
      ...
      expect(consoleSpy.debug).toHaveBeenCalledWith(
        "StatusProvider: Cache miss for Step, fetching from API",
      );
    ...
    test("should return fallback statuses when API fails", async () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "StatusProvider: Using fallback statuses for Step",
      );
      expect(mockSecurityUtils.logSecurityEvent).toHaveBeenCalledWith(
        "StatusProvider:api_fetch_error",
        expect.any(Object),
      );
    });
  ...
  describe("Security Features - Input Validation", () => {
    test("should validate entity types against whitelist", async () => {
      ...
      expect(result).toEqual([]);
      expect(mockSecurityUtils.logSecurityEvent).toHaveBeenCalledWith(
        "StatusProvider:invalid_entity_type",
        {
          entityType: "InvalidType",
          errors: [
            "Invalid entity type. Must be one of: Step, Phase, Sequence, Iteration, Plan, Migration, Control",
          ],
        },
      );
    });
    ...
    test("validateEntityType should return correct validation results", () => {
      ...
      // Invalid entity type
      const invalidResult =
        statusProvider.validateEntityType("HackerType<script>");
  ...
  describe("Security Features - XSS Prevention", () => {
    test("should sanitize malicious input strings", () => {
      ...
      expect(mockSecurityUtils.sanitizeXSS).toHaveBeenCalledWith(
        maliciousInput,
      );
    });
    ...
    test("should sanitize CSS values to prevent CSS injection", () => {
      ...
      expect(mockSecurityUtils.sanitizeForCSS).toHaveBeenCalledWith(
        maliciousCSS,
      );
    });
    ...
    test("should sanitize color values correctly", () => {
      ...
      expect(statusProvider.sanitizeColor("rgb(255, 0, 0)")).toBe(
        "rgb(255, 0, 0)",
      );
      expect(statusProvider.sanitizeColor("rgba(255, 0, 0, 0.5)")).toBe(
        "rgba(255, 0, 0, 0.5)",
      );
  ...
  describe("ETag Support", () => {
    ...
    test("should handle 304 Not Modified responses", async () => {
      ...
      expect(consoleSpy.debug).toHaveBeenCalledWith(
        "StatusProvider: Data not modified (304), using cached version",
      );
    });
  });
  ...
  describe("Dropdown Options", () => {
    test("should get cached dropdown options when available", async () => {
      ...
      expect(consoleSpy.debug).toHaveBeenCalledWith(
        "StatusProvider: Cache hit for dropdown options Step",
      );
    });
    ...
    test("should return fallback dropdown options when API fails", async () => {
      ...
      expect(result).toEqual([
        {
          value: mockStatusProvider.getStatusNameById(1),
          text: "Pending",
          cssClass: "status-pending",
        },
        {
          value: mockStatusProvider.getStatusNameById(7),
          text: "To Do",
          cssClass: "status-todo",
        },
        {
          value: mockStatusProvider.getStatusNameById(2),
          text: "In Progress",
          cssClass: "status-in-progress",
        },
        {
          value: mockStatusProvider.getStatusNameById(3),
          text: "Completed",
          cssClass: "status-completed",
        },
        {
          value: mockStatusProvider.getStatusNameById(4),
          text: "Failed",
          cssClass: "status-failed",
        },
        {
          value: mockStatusProvider.getStatusNameById(6),
          text: "Blocked",
          cssClass: "status-blocked",
        },
        {
          value: mockStatusProvider.getStatusNameById(5),
          text: "Cancelled",
  ...
  describe("Get All Statuses", () => {
    test("should get cached all statuses when available", async () => {
      const cachedAllStatuses = {
        Step: [{ name: mockStatusProvider.getStatusNameById(1) }],
        Phase: [{ name: "PLANNING" }],
      };
      ...
      expect(consoleSpy.debug).toHaveBeenCalledWith(
        "StatusProvider: Cache hit for all statuses",
      );
    });
    ...
    test("should fetch all statuses from API", async () => {
      const apiResponse = {
        statusesByType: {
          Step: [{ name: mockStatusProvider.getStatusNameById(1), id: 1 }],
    ...
    test("should return fallback statuses when API fails", async () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "StatusProvider: Using all fallback statuses",
      );
    });
  });
  ...
  describe("DOM Manipulation Security", () => {
    test("should populate dropdown with XSS protection", async () => {
      ...
      await statusProvider.populateStatusDropdown(
        mockSelectElement,
        "Step",
    ...
    test("should validate select element before populating", async () => {
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith(
        "StatusProvider: Valid select element is required",
      );
    ...
    test("should handle dropdown population errors gracefully", async () => {
      statusProvider.getDropdownOptions = jest
        .fn()
      ...
      expect(mockSecurityUtils.logSecurityEvent).toHaveBeenCalledWith(
        "StatusProvider:dropdown_population_error",
        expect.any(Object),
      );
    ...
    test("should preserve existing placeholder option", async () => {
      ...
      expect(mockSelectElement.appendChild).toHaveBeenCalledWith(
        placeholderOption,
      );
    });
  });
  ...
  describe("Utility Methods", () => {
    test("should format status display names correctly", () => {
      expect(
        statusProvider.formatStatusDisplay(
      ...
      expect(statusProvider.formatStatusDisplay("SNAKE_CASE_STATUS")).toBe(
        "Snake Case Status",
      );
    ...
    test("should generate CSS class names safely", () => {
      expect(
        statusProvider.getStatusCssClass(
          mockStatusProvider.getStatusNameById(2),
        ),
      ...
      expect(statusProvider.getStatusCssClass("TEST<script>")).toBe(
        "status-testscript",
      );
    ...
    test("should generate consistent mock IDs", () => {
      const id1 = statusProvider.generateMockId(
        mockStatusProvider.getStatusNameById(1),
        "Step",
      );
      const id2 = statusProvider.generateMockId(
        mockStatusProvider.getStatusNameById(1),
        "Step",
      );
      const id3 = statusProvider.generateMockId(
    ...
    test("should generate dropdown options from statuses array", () => {
      const statuses = [
        { name: mockStatusProvider.getStatusNameById(1), color: "#ffff00" },
        { name: mockStatusProvider.getStatusNameById(3), color: "#00ff00" },
    ...
    test("should handle invalid input in generateDropdownOptions", () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "StatusProvider: Invalid statuses array provided to generateDropdownOptions",
      );
    });
  });
  ...
  describe("Fallback Status Handling", () => {
    ...
    test("should return empty array for invalid fallback entity types", () => {
      ...
      expect(result).toEqual([]);
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "StatusProvider: Invalid entity type for fallback statuses",
      );
    });
    ...
    test("should handle missing fallback data gracefully", () => {
      ...
      expect(result).toEqual([]);
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "StatusProvider: No fallback statuses for entity type Step",
      );
  ...
  describe("Error Handling and Logging", () => {
    test("should sanitize data for logging safely", () => {
      expect(statusProvider.sanitizeForLogging("normal text")).toBe(
        "normal text",
      );
      expect(
        statusProvider.sanitizeForLogging("<script>alert(1)</script>"),
      ).toBe("scriptalert(1)/script");
      expect(statusProvider.sanitizeForLogging(null)).toBe("[null/undefined]");
      expect(statusProvider.sanitizeForLogging(undefined)).toBe(
        "[null/undefined]",
      );
      expect(statusProvider.sanitizeForLogging({ key: "value" })).toBe(
        "[object]",
      );
    ...
    test("should sanitize password/token data for logging", () => {
      expect(statusProvider.sanitizeForLogging("password=secret123")).toBe(
        "[REDACTED]=[REDACTED]123",
      );
      expect(statusProvider.sanitizeForLogging("authToken=abc123")).toBe(
        "[REDACTED][REDACTED]=abc123",
      );
      expect(statusProvider.sanitizeForLogging("API_KEY=xyz789")).toBe(
        "API_[REDACTED]=xyz789",
      );
    });

    test("should sanitize errors for logging", () => {
      const error = new Error("Database connection failed");
      expect(statusProvider.sanitizeErrorForLogging(error)).toBe(
        "Database connection failed",
      );

      const errorWithoutMessage = {};
      expect(statusProvider.sanitizeErrorForLogging(errorWithoutMessage)).toBe(
    ...
    test("should fallback to console logging when SecurityUtils unavailable", () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "[StatusProvider Security] test_event:",
        {
          detail: "test_detail",
  ...
  describe("Integration with SecurityUtils", () => {
    ...
    test("should work completely without SecurityUtils", async () => {
      ...
      global.fetch.mockResolvedValue({
        ...
        status: 200,
        json: () =>
          Promise.resolve({ statuses: [{ name: "TEST", color: "red" }] }),
        headers: { get: () => null },
      });
