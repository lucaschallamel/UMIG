...
class CachingTeamsEntityManager {
    ...
    async withRetry(operation, operationName = 'operation') {
        ...
        for (let attempt = 0; attempt <= this.retryConfig.maxRetries; attempt++) {
            try {
                ...
                // Log retry attempt
                console.warn(`Retry ${attempt + 1}/${this.retryConfig.maxRetries + 1} for ${operationName}:`, error.message);
            }
        }
        ...
        if (this.errorRecoveryCallbacks.has(operationName)) {
            try {
                ...
                console.error(`Error recovery failed for ${operationName}:`, recoveryError);
            }
        }
    ...
    async loadData(filters = {}, sort = null, page = 1, pageSize = 20) {
        ...
        const result = await this.withRetry(async () => {
            ...
            if (!response.ok) {
                const error = new Error(`Failed to fetch teams: ${response.status} ${await response.text()}`);
    ...
    getPerformanceStats(operation) {
        ...
        const cacheMisses = metrics.filter(m => m.metadata.cache === 'miss').length;
...
describe('Teams Performance and Caching Tests', () => {
    ...
    describe('Caching Strategy Tests', () => {
        test('should cache successful API responses', async () => {
            const mockResponse = { teams: [{ id: '1', name: 'Test Team' }], total: 1 };
    ...
    describe('Error Recovery and Retry Tests', () => {
        ...
        test('should not retry client errors (4xx)', async () => {
            ...
            await expect(teamsManager.loadData())
                .rejects.toThrow('Failed to fetch teams: 400 Bad Request');
        ...
        test('should execute error recovery callbacks', async () => {
            ...
            await expect(teamsManager.loadData())
                .rejects.toThrow('Failed to fetch teams: 500 Server Error');
    ...
    describe('Performance Regression Tests', () => {
        ...
        test('should demonstrate performance improvement with caching', async () => {
            const mockResponse = { teams: TeamBuilder.performanceDataset(50), total: 50 };
    ...
    describe('Network Resilience Tests', () => {
        test('should handle intermittent network failures', async () => {
            ...
            global.fetch = jest.fn(() => responses[callCount++] || responses[responses.length - 1]);
        ...
        test('should handle request cancellation gracefully', async () => {
            ...
            global.fetch = jest.fn(() => Promise.reject(new Error('Request cancelled')));
