...
const REGRESSION_THRESHOLDS = {
  warning: 0.1, // 10% degradation triggers warning
  critical: 0.2, // 20% degradation triggers failure
...
class PerformanceRegessionTracker {
      ...
      switch (analysis.status) {
        case "critical":
          ...
          report.recommendations.push(
            `CRITICAL: ${operation} - ${analysis.message}`,
          );
          break;
        case "warning":
          report.summary.warningRegressions++;
          report.recommendations.push(
            `WARNING: ${operation} - ${analysis.message}`,
          );
          break;
        case "excellent":
        case "good":
  ...
  analyzeTrends() {
    ...
    for (const [operation] of this.measurements) {
      ...
      const earlierAvg =
        earlier.reduce((sum, m) => sum + m.duration, 0) / earlier.length;
...
describe("Teams Performance Regression Tests", () => {
    ...
    test("should detect performance regressions", async () => {
      ...
      for (let i = 0; i < 15; i++) {
        performanceTracker.addMeasurement(
          "load",
          regressedDuration + Math.random() * 50,
        );
      }
      ...
      expect(analysis.regressionFromTarget).toBeGreaterThan(
  ...
  describe("Memory Performance Regression", () => {
    test("should detect memory usage regressions", async () => {
      const mockResponse = {
        teams: TeamBuilder.performanceDataset(1000),
        total: 1000,
      };
      global.fetch = jest.fn(() =>
        Promise.resolve({
  ...
  describe("Regression Detection Algorithms", () => {
    test("should use statistical significance for regression detection", async () => {
      ...
      const normalPerformance = Array.from(
        { length: 30 },
      ...
      expect(Math.abs(stats.median - PERFORMANCE_TARGETS.load)).toBeLessThan(
        20,
      );
      expect(stats.standardDeviation).toBeGreaterThan(0);
    });
  ...
  describe("Performance Alerting and Recommendations", () => {
    test("should provide actionable performance recommendations", async () => {
      ...
      expect(analysis.status).toBe("critical");
      expect(analysis.recommendation).toContain("URGENT");
      expect(analysis.recommendation).toMatch(
        /profiling|caching|optimization/i,
      );
    });
