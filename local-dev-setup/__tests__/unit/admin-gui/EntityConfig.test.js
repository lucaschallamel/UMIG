...
describe("EntityConfig Module", () => {
  ...
  describe("Entity Retrieval", () => {
    ...
    test("should retrieve iterationTypes entity with complete configuration", () => {
      ...
      expect(iterationTypes.description).toBe(
        "Manage iteration types and their configurations",
      );
      expect(iterationTypes.fields).toBeInstanceOf(Array);
      expect(iterationTypes.fields.length).toBeGreaterThan(0);

      // Check for required fields
      const codeField = iterationTypes.fields.find(
        (field) => field.key === "itt_code",
      );
      expect(codeField).toBeDefined();
      expect(codeField.required).toBe(true);
      expect(codeField.readonly).toBe(false);

      const nameField = iterationTypes.fields.find(
        (field) => field.key === "itt_name",
      );
      expect(nameField).toBeDefined();
      expect(nameField.required).toBe(true);

      const colorField = iterationTypes.fields.find(
        (field) => field.key === "itt_color",
      );
    ...
    test("should retrieve migrationTypes entity with complete configuration", () => {
      ...
      expect(migrationTypes.description).toBe(
        "Manage migration types and their configurations",
      );
      ...
      const idField = migrationTypes.fields.find(
        (field) => field.key === "mit_id",
      );
      ...
      // Check for required fields
      const codeField = migrationTypes.fields.find(
        (field) => field.key === "mit_code",
      );
      expect(codeField).toBeDefined();
      expect(codeField.required).toBe(true);
      expect(codeField.readonly).toBe(false);

      const nameField = migrationTypes.fields.find(
        (field) => field.key === "mit_name",
      );
      expect(nameField).toBeDefined();
      expect(nameField.required).toBe(true);
    });
  ...
  describe("Field Configuration Validation", () => {
    ...
    test("color fields should have valid default values", () => {
      ...
      const iterationColorField = iterationTypes.fields.find(
        (field) => field.type === "color",
      );
      const migrationColorField = migrationTypes.fields.find(
        (field) => field.type === "color",
      );
    ...
    test("boolean fields should have valid default values", () => {
      ...
      const iterationActiveField = iterationTypes.fields.find(
        (field) => field.key === "itt_active",
      );
      const migrationActiveField = migrationTypes.fields.find(
        (field) => field.key === "mit_active",
      );
