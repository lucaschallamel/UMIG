...
const TEST_CONFIG = {
  ...
  stepViewUrl:
    "/spaces/UMIG/pages/1114120/UMIG+-+Step+View?mig=TORONTO&ite=RUN1&stepid=BGO-002",
  timeout: 10000,
  screenshotPath:
    "/Users/lucaschallamel/Documents/GitHub/UMIG/src/groovy/umig/tests/integration/screenshots/",
};
...
test.describe("StepView JavaScript-HTML Synchronization", () => {
  ...
  test("should verify UMIG_STEP_CONFIG is properly initialized", async ({
    page,
  }) => {
    ...
    console.log(
      `✅ User configuration validated - Role: ${configStructure.userRole}`,
    );
  ...
  test("should verify HTML structure matches expected CSS classes", async ({
    page,
  }) => {
  ...
  test("should verify iteration-view.css is loaded and applied", async ({
    page,
  }) => {
    ...
    if (cssLoaded) {
      ...
      console.log(
        "⚠️ iteration-view.css not found - checking for inline styles",
      );
    }
    ...
    const containerStyles = await page
      .locator(".step-details-container")
      .evaluate((el) => {
  ...
  test("should verify JavaScript DOM manipulation works correctly", async ({
    page,
  }) => {
    ...
    await page.evaluate((data) => {
      ...
      if (instructionCount)
        instructionCount.textContent = `(${data.instructions.length})`;
    ...
    const populatedData = await page.evaluate(() => {
      return {
        ...
        instructionCount:
          document.querySelector(".instructions-container .instruction-item")
            ?.length || 0,
        commentCount:
          document.querySelector(".comments-container .comment-item")?.length ||
          0,
        instructionCountDisplay:
          document.querySelector(".instruction-count")?.textContent,
        commentCountDisplay:
    ...
    expect(populatedData.instructionCount).toBe(
      mockStepData.instructions.length,
    );
  ...
  test("should verify role-based UI elements display correctly", async ({
    page,
  }) => {
    ...
    const updateStatusVisible = await page
      .locator("button.update-status, .update-status")
      .isVisible();
    const bulkCompleteVisible = await page
      .locator("button.bulk-complete, .bulk-complete")
      .isVisible();
    ...
    if (userConfig.role === "NORMAL") {
      ...
      console.log(
        "✅ PILOT user correctly has update status but not bulk complete",
      );
    ...
    console.log(
      `✅ Bulk operations config (${userConfig.bulkOperations}) matches isPilot (${userConfig.isPilot})`,
    );
    ...
    await page.screenshot({
      path:
        ...
        `stepview-${userConfig.role.toLowerCase()}-ui.png`,
      fullPage: true,
    });
  });

  test("should verify caching and real-time sync configuration", async ({
    page,
  }) => {
    ...
    if (cacheAvailable) {
      ...
      console.log(
        "⚠️ StepViewCache class not found - checking global configuration",
      );
    }
