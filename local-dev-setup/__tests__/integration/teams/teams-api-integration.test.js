...
describe("Teams API Integration Tests - Real Endpoints", () => {
  ...
  describe("CRUD Operations - Real API", () => {
    test(
      "should create team via real API endpoint",
      async () => {
    ...
    test(
      "should retrieve teams via real API endpoint",
      async () => {
    ...
    test(
      "should update team via real API endpoint",
      async () => {
        const testTeam = await createTestTeam();
    ...
    test(
      "should delete team via real API endpoint",
      async () => {
        const testTeam = await createTestTeam();
    ...
    test(
      "should handle team not found gracefully",
      async () => {
    ...
    test(
      ...
      async () => {
        const response = await fetch(
          `${BASE_URL}/teams?sortBy=name&sortDir=asc`,
          {
            headers: authHeaders,
          },
        );
  ...
  describe("Team Members - Real API Integration", () => {
    test(
      "should load team members via API",
      async () => {
        const testTeam = await createTestTeam();

        const response = await fetch(
          `${BASE_URL}/team-members?teamId=${testTeam.id}`,
          {
            headers: authHeaders,
          },
        );
    ...
    test(
      "should assign member to team via API",
      async () => {
        const testTeam = await createTestTeam();
    ...
    test(
      "should remove member from team via API",
      async () => {
        const testTeam = await createTestTeam();
        const testUser = await createTestUser();
        ...
        const response = await fetch(
          `${BASE_URL}/team-members/${testTeam.id}/${testUser.userId}`,
          {
            method: "DELETE",
            headers: authHeaders,
          },
  ...
  describe("Bulk Operations - Real API Integration", () => {
    test(
      "should perform bulk delete via API",
      async () => {
        // Create multiple test teams
        const teams = await Promise.all([
    ...
    test(
      "should perform bulk status update via API",
      async () => {
        // Create test teams
        const teams = await Promise.all([
  ...
  async function createTestTeam(name = null, status = "active") {
    const teamData = new TeamBuilder()
      .withName(
        name ||
          `Test Team ${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      )
      .withDescription("Created by integration test")
      .withStatus(status)
      .build();
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to create test team: ${response.status} ${await response.text()}`,
      );
    }
