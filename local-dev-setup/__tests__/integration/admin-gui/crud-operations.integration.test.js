...
test.describe('Admin GUI CRUD Operations Integration', () => {
  ...
  test.describe('Iteration Types CRUD Operations', () => {
    ...
    test('should generate correct form fields for Iteration Types', async ({ page }) => {
      await page.click('#add-iterationTypes');
      await page.waitForSelector('.modal', { timeout: 3000 });
      ...
      for (const field of expectedFields) {
        ...
        // Verify field labels
        const labelExists = await page.locator(`label[for="${field.id}"]`).isVisible();
        expect(labelExists).toBe(true);
      }
    });

    test('should validate required fields for Iteration Types', async ({ page }) => {
      await page.click('#add-iterationTypes');
      await page.waitForSelector('.modal', { timeout: 3000 });
      ...
      const invalidFieldExists = await codeField.isVisible() || 
                                  await nameField.isVisible() || 
                                  await colorField.isVisible();
      expect(invalidFieldExists).toBe(true);
    });

    test('should set default values for Iteration Types fields', async ({ page }) => {
      await page.click('#add-iterationTypes');
      await page.waitForSelector('.modal', { timeout: 3000 });
    ...
    test('should create new Iteration Type successfully', async ({ page }) => {
      ...
      const successNotification = await page.locator('.notification-success').isVisible();
      expect(successNotification).toBe(true);
    });

    test('should handle Edit operation for Iteration Types', async ({ page }) => {
    ...
    test('should make itt_code readonly in Edit mode', async ({ page }) => {
      ...
      const codeFieldDisabled = await page.locator('#itt_code:disabled').isVisible();
  ...
  test.describe('Migration Types CRUD Operations', () => {
    ...
    test('should generate correct form fields for Migration Types', async ({ page }) => {
      await page.click('#add-migrationTypes');
      await page.waitForSelector('.modal', { timeout: 3000 });
      
      // Verify required form fields are present (note: mtm_id should not be visible in create mode)
      const expectedFields = [
        { id: 'mtm_code', label: 'Code' },
        { id: 'mtm_name', label: 'Name' },
        { id: 'mtm_description', label: 'Description' },
        { id: 'mtm_color', label: 'Color' },
        { id: 'mtm_icon', label: 'Icon' },
        { id: 'mtm_display_order', label: 'Display Order' },
        { id: 'mtm_active', label: 'Active' }
      ];
      ...
      // mtm_id should not be visible in create mode (readonly primary key)
      const idFieldVisible = await page.locator('#mtm_id').isVisible();
      expect(idFieldVisible).toBe(false);
    });

    test('should validate required fields for Migration Types', async ({ page }) => {
      await page.click('#add-migrationTypes');
      await page.waitForSelector('.modal', { timeout: 3000 });
      ...
      const codeField = await page.locator('#mtm_code:invalid');
      const nameField = await page.locator('#mtm_name:invalid');
      const colorField = await page.locator('#mtm_color:invalid');
    ...
    test('should set default values for Migration Types fields', async ({ page }) => {
      await page.click('#add-migrationTypes');
      await page.waitForSelector('.modal', { timeout: 3000 });
      
      // Check default values
      const colorValue = await page.inputValue('#mtm_color');
      expect(colorValue).toBe('#6B73FF');
      
      const iconValue = await page.inputValue('#mtm_icon');
      expect(iconValue).toBe('migration');
      
      const displayOrderValue = await page.inputValue('#mtm_display_order');
      expect(displayOrderValue).toBe('0');
      
      const activeChecked = await page.isChecked('#mtm_active');
      expect(activeChecked).toBe(true);
    });
    ...
    test('should create new Migration Type successfully', async ({ page }) => {
        ...
        if (route.request().method() === 'POST') {
          route.fulfill({
            ...
            body: JSON.stringify({
              ...
              mtm_description: 'Test Migration Description',
              mtm_color: '#FF6B6B',
              mtm_icon: 'test-migration',
              mtm_display_order: 1,
              mtm_active: true,
              created_at: new Date().toISOString(),
              created_by: 'testuser'
      ...
      // Fill form
      await page.fill('#mtm_code', 'TESTMIG');
      await page.fill('#mtm_name', 'Test Migration');
      await page.fill('#mtm_description', 'Test Migration Description');
      await page.fill('#mtm_color', '#FF6B6B');
      await page.fill('#mtm_icon', 'test-migration');
    ...
    test('should show ID field in Edit mode for Migration Types', async ({ page }) => {
      // Mock existing data
      await page.route('**/migrationTypes', (route) => {
        route.fulfill({
          status: 200,
      ...
      const idFieldDisabled = await page.locator('#mtm_id:disabled').isVisible();
      expect(idFieldDisabled).toBe(true);
      
      const idValue = await page.inputValue('#mtm_id');
  ...
  test.describe('Form Field Validation', () => {
    ...
    test('should validate color field format', async ({ page }) => {
      ...
      const colorFieldInvalid = await page.locator('#itt_color:invalid').isVisible();
    ...
    test('should validate number field types', async ({ page }) => {
      ...
      const numberFieldInvalid = await page.locator('#itt_display_order:invalid').isVisible();
  ...
  test.describe('Error Handling', () => {
    ...
    test('should handle server errors during creation', async ({ page }) => {
      ...
      const errorNotification = await page.locator('.notification-error').isVisible();
    ...
    test('should handle validation errors from server', async ({ page }) => {
      ...
      // Fill form
      await page.fill('#mtm_code', 'EXISTING');
      await page.fill('#mtm_name', 'Test Name');
      await page.fill('#mtm_color', '#FF0000');
      
      await page.click('.modal .btn-primary');
      await page.waitForTimeout(2000);
      
      // Should show validation error
      const errorMessage = await page.locator('.error-message, .notification-error').isVisible();
