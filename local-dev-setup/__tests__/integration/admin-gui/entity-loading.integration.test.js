...
test.describe("Admin GUI Entity Loading Integration", () => {
  ...
  test.describe("EntityConfig Integration", () => {
    ...
    test("should have iterationTypes configuration available", async ({
      page,
    }) => {
    ...
    test("should have migrationTypes configuration available", async ({
      page,
    }) => {
  ...
  test.describe("Labels Section Loading (Baseline)", () => {
    test("should load Labels section without errors", async ({ page }) => {
      ...
      const activeSection = await page.getAttribute(
        'button[data-section="labels"]',
        "class",
      );
  ...
  test.describe("Iteration Types Section Loading", () => {
    test("should load Iteration Types section without errors", async ({
      page,
    }) => {
      // Track console errors
      const consoleErrors = [];
      ...
      const activeSection = await page.getAttribute(
        'button[data-section="iterationTypes"]',
        "class",
      );
      expect(activeSection).toContain("active");
    });
    ...
    test("should display Iteration Types table", async ({ page }) => {
      ...
      for (const column of expectedColumns) {
        const columnExists = await page
          .locator(`#iterationTypes-table thead th:has-text("${column}")`)
          .isVisible();
    ...
    test("should enable Add button for Iteration Types", async ({ page }) => {
      ...
      const addButtonExists = await page
        .locator("#add-iterationTypes")
        .isVisible();
      expect(addButtonExists).toBe(true);

      const addButtonEnabled = await page
        .locator("#add-iterationTypes")
        .isEnabled();
    ...
    test("should display existing iteration types data", async ({ page }) => {
      await page.click('button[data-section="iterationTypes"]');
      await page.waitForSelector("#iterationTypes-table tbody", {
        timeout: 5000,
      });

      // Check if table has data rows
      const rowCount = await page
        .locator("#iterationTypes-table tbody tr")
        .count();
      ...
      if (rowCount > 0) {
        const firstRowCells = await page
          .locator("#iterationTypes-table tbody tr:first-child td")
          .count();
  ...
  test.describe("Migration Types Section Loading", () => {
    test("should load Migration Types section without errors", async ({
      page,
    }) => {
      // Track console errors
      const consoleErrors = [];
      ...
      const activeSection = await page.getAttribute(
        'button[data-section="migrationTypes"]',
        "class",
      );
      expect(activeSection).toContain("active");
    });
    ...
    test("should display Migration Types table", async ({ page }) => {
      ...
      for (const column of expectedColumns) {
        const columnExists = await page
          .locator(`#migrationTypes-table thead th:has-text("${column}")`)
          .isVisible();
        expect(columnExists).toBe(true);
      }
    });
    ...
    test("should display existing migration types data", async ({ page }) => {
      await page.click('button[data-section="migrationTypes"]');
      await page.waitForSelector("#migrationTypes-table tbody", {
        timeout: 5000,
      });

      // Check if table has data rows
      const rowCount = await page
        .locator("#migrationTypes-table tbody tr")
        .count();
      // Should have at least some test data or empty state
      expect(rowCount).toBeGreaterThanOrEqual(0);
      ...
      if (rowCount > 0) {
        const firstRowCells = await page
          .locator("#migrationTypes-table tbody tr:first-child td")
          .count();
  ...
  test.describe("Section Navigation Performance", () => {
    ...
    test("should maintain section state during navigation", async ({
      page,
    }) => {
      // Load iterationTypes
      await page.click('button[data-section="iterationTypes"]');
  ...
  test.describe("Error Handling", () => {
    ...
    test("should display appropriate error messages for failed loads", async ({
      page,
    }) => {
      ...
      const errorMessage = await page
        .locator(".error-message, .empty-state, .notification-error")
        .isVisible();
