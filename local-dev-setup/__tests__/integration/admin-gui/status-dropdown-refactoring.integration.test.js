...
describe("Status Dropdown Refactoring Tests", () => {
  ...
  beforeEach(() => {
    ...
    iterationView = {
      fetchStepStatuses: jasmine
        .createSpy("fetchStepStatuses")
        .and.returnValue(Promise.resolve(mockStatuses)),
      ...
      userRole: "PILOT",
      apiClient: {
        updateStepStatus: jasmine.createSpy("updateStepStatus").and.returnValue(
          Promise.resolve({
            success: true,
            emailsSent: 2,
            responseTime: 150,
    ...
    iterationView.populateStatusDropdown =
      window.IterationView.prototype.populateStatusDropdown.bind(iterationView);
    iterationView.handleStatusChange =
      window.IterationView.prototype.handleStatusChange.bind(iterationView);
  });
  ...
  describe("Error Handling", () => {
    it("should handle fetchStepStatuses failure gracefully", async () => {
      iterationView.fetchStepStatuses.and.returnValue(
    ...
    it("should handle API update failure gracefully", async () => {
      iterationView.apiClient.updateStepStatus.and.returnValue(
      ...
      // Should not throw error
      await expectAsync(
        iterationView.handleStatusChange(event),
      ).not.toBeRejected();
    });
  });
