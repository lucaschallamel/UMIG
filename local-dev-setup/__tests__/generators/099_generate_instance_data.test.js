...
describe("Instance Data Generator (99_generate_instance_data.js)", () => {
  ...
  describe("generateInstanceData", () => {
    ...
    it("should call eraseInstanceDataTables when erase option is true", async () => {
      ...
      const truncateCall = client.query.mock.calls.find((call) =>
        call[0].includes("TRUNCATE"),
      );
    ...
    it("should throw an error if no master iterations are found", async () => {
      ...
      await expect(generateInstanceData(CONFIG)).rejects.toThrow(
    ...
    it("should include override fields in INSERT statements", async () => {
      const mockData = {
        sequences: {
          rows: [
            {
              sqm_id: "sqm-1",
              sqm_name: "Original Name",
              sqm_description: "Original Description",
              sqm_order: 1,
        ...
        phases: {
          rows: [
            {
              phm_id: "phm-1",
              phm_name: "Original Phase",
              phm_description: "Original Phase Description",
              phm_order: 1,
        ...
        steps: {
          rows: [
            {
              stm_id: "stm-1",
              stm_name: "Original Step",
              stm_description: "Original Step Description",
          ...
          rows: [
            {
              ctm_id: "ctm-1",
              ctm_order: 1,
              ctm_name: "Original Control",
              ctm_description: "Original Control Description",
              ctm_type: "CHECK",
      ...
      const planInsert = sqlQueries.find((sql) =>
        sql.includes("INSERT INTO plans_instance_pli"),
      );
      expect(planInsert).toContain("pli_status");

      const sequenceInsert = sqlQueries.find((sql) =>
        sql.includes("INSERT INTO sequences_instance_sqi"),
      );
      ...
      const phaseInsert = sqlQueries.find((sql) =>
        sql.includes("INSERT INTO phases_instance_phi"),
      );
      ...
      const stepInsert = sqlQueries.find((sql) =>
        sql.includes("INSERT INTO steps_instance_sti"),
      );
      ...
      const instructionInsert = sqlQueries.find((sql) =>
        sql.includes("INSERT INTO instructions_instance_ini"),
      );
      ...
      const controlInsert = sqlQueries.find((sql) =>
        sql.includes("INSERT INTO controls_instance_cti"),
      );
  ...
  describe("eraseInstanceDataTables", () => {
    ...
    it("should truncate tables in the correct order", async () => {
      ...
      tablesToReset.forEach((table) => {
        expect(mockQueryImplementation).toHaveBeenCalledWith(
    ...
    it("should skip truncation if a table does not exist", async () => {
      ...
      client.query.mockImplementation(mockQueryImplementation);
      await eraseInstanceDataTables(client);
      expect(mockQueryImplementation).not.toHaveBeenCalledWith(
        `TRUNCATE TABLE "plans_instance_pli" RESTART IDENTITY CASCADE`,
      );
      tablesToReset
        .filter((table) => table !== "plans_instance_pli")
        .forEach((table) => {
          expect(mockQueryImplementation).toHaveBeenCalledWith(
            `TRUNCATE TABLE "${table}" RESTART IDENTITY CASCADE`,
          );
