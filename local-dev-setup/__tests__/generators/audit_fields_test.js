...
// Test configuration
const TEST_CONFIG = {
  connectionString: process.env.DATABASE_URL || 'postgresql://umig_user:umig_password@localhost:5432/umig_db',
  testTimeout: 30000,
...
describe('Generator Scripts - Audit Fields Population', () => {
  ...
  describe('Database Setup Verification', () => {
    ...
    test('should have audit fields migration applied', async () => {
      ...
      console.log(`✅ Found ${auditColumnCount} audit columns across key tables`);
    });
  });
  ...
  describe('Updated Generators - Audit Field Population', () => {
    test('should populate audit fields in migrations_mig table', async () => {
      ...
      console.log(`✅ Verified audit fields in ${result.rows.length} migration records`);
    });
    ...
    test('should populate audit fields in iterations_ite table', async () => {
      ...
      console.log(`✅ Verified audit fields in ${result.rows.length} iteration records`);
    });
    ...
    test('should populate audit fields in canonical plans (sequences, phases, steps)', async () => {
      ...
      for (const { table, id, name } of tables) {
        ...
        if (result.rows.length === 0) {
          console.log(`ℹ️  No records found in ${table} - run generators first`);
          continue;
        }
        ...
        console.log(`  ✅ ${table}: ${result.rows.length} records with correct audit fields`);
      }
    });
    ...
    test('should populate audit fields in instance data tables', async () => {
      ...
      for (const table of instanceTables) {
        ...
        try {
          ...
          console.log(`  ✅ ${table}: ${totalRecords} records with correct audit fields`);
          
        } catch (error) {
          console.log(`  ⚠️  ${table}: Table might not exist or have audit fields yet`);
        }
      }
    });
    ...
    test('should populate audit fields in labels and related linking tables', async () => {
      const labelQueries = [
        {
          name: 'labels_lbl',
          query: 'SELECT created_by, updated_by, created_at, updated_at FROM labels_lbl WHERE created_by IS NOT NULL LIMIT 3'
        },
        {
          name: 'labels_lbl_x_steps_master_stm', 
          query: 'SELECT created_by FROM labels_lbl_x_steps_master_stm WHERE created_by IS NOT NULL LIMIT 3'
        },
        {
          name: 'labels_lbl_x_applications_app',
          query: 'SELECT created_by FROM labels_lbl_x_applications_app WHERE created_by IS NOT NULL LIMIT 3'
        }
      ];
      ...
      for (const { name, query } of labelQueries) {
        try {
          ...
          console.log(`  ✅ ${name}: ${result.rows.length} records with correct audit fields`);
  ...
  describe('Generators Needing Updates - Expected Failures/Warnings', () => {
    test('should identify generators that need audit field updates', async () => {
      ...
      for (const { table, generator, auditFields } of generatorsNeedingUpdates) {
        ...
        try {
          ...
          if (totalRecords > 0 && auditRecords === 0) {
            console.log(`      Status: ${totalRecords} records without audit fields ❌`);
          } else if (totalRecords > 0 && auditRecords > 0) {
            console.log(`      Status: ${auditRecords}/${totalRecords} records have audit fields ⚠️`);
          } else {
    ...
    test('should verify teams_tms table needs audit field population', async () => {
      try {
        ...
        if (parseInt(total_count) > 0) {
          const needsUpdate = parseInt(has_created_by) === 0 || parseInt(has_updated_by) === 0;
          
          if (needsUpdate) {
            console.log(`⚠️  teams_tms: ${total_count} records need audit field population`);
            console.log(`   Current: ${has_created_by} have created_by, ${has_updated_by} have updated_by`);
          } else {
            console.log(`✅ teams_tms: ${total_count} records already have audit fields`);
          }
        } else {
    ...
    test('should verify applications_app table needs audit field population', async () => {
      try {
        ...
        if (parseInt(total_count) > 0) {
          ...
          if (needsUpdate) {
            console.log(`⚠️  applications_app: ${total_count} records need audit field population`);
          } else {
            console.log(`✅ applications_app: ${total_count} records already have audit fields`);
          }
        } else {
        ...
        console.log(`❌ applications_app audit field check failed: ${error.message}`);
        throw error;
      }
    });
  });
    ...
    test('should have consistent timestamp relationships (created_at <= updated_at)', async () => {
      ...
      for (const table of tablesWithData) {
        try {
          ...
          if (result.rows.length > 0) {
            const { total_records, valid_timestamps, invalid_timestamps } = result.rows[0];
            
            if (parseInt(total_records) > 0) {
              expect(parseInt(invalid_timestamps)).toBe(0);
              expect(parseInt(valid_timestamps)).toBe(parseInt(total_records));
              
              console.log(`  ✅ ${table}: ${total_records} records with valid timestamp relationships`);
            }
          }
        } catch (error) {
          console.log(`  ⚠️  ${table}: Timestamp check failed - ${error.message}`);
        }
      }
    });
        ...
        try {
          ...
          if (result.rows.length > 0) {
            ...
            if (totalRecords > 0) {
              ...
              console.log(`  ✅ ${table}: ${totalRecords} records with complete audit fields`);
            }
          }
        } catch (error) {
          console.log(`  ⚠️  ${table}: Completeness check failed - ${error.message}`);
        }
      }
    });
      ...
      for (const table of updatedTables) {
        try {
          ...
          if (result.rows.length > 0) {
            ...
            const generatorRecords = result.rows.find(row => row.created_by === TEST_CONFIG.expectedAuditUser);
            if (generatorRecords) {
              console.log(`  ✅ ${table}: ${generatorRecords.count} records created by '${TEST_CONFIG.expectedAuditUser}'`);
            }
          }
        } catch (error) {
          console.log(`  ⚠️  ${table}: Audit user check failed - ${error.message}`);
        }
      }
    ...
    test('should verify audit field indexes exist and perform well', async () => {
      const indexQueries = [
        ...
        },
        {
          ...
          query: "SELECT COUNT(*) FROM migrations_mig WHERE created_at > CURRENT_TIMESTAMP - INTERVAL '1 day'"
        },
        {
  ...
  describe('Recommendations and Next Steps', () => {
    test('should provide update recommendations for remaining generators', async () => {
      const recommendations = [
        ...
        },
        {
          ...
          tables: ['users_usr'],
          updateNeeded: 'Add created_by and updated_by (timestamps already exist)',
          priority: 'MEDIUM',
          pattern: `
    ...
    test('should verify AuditFieldsUtil integration opportunities', async () => {
      ...
      const integrationOpportunities = [
        {
          generator: '002_generate_teams_apps.js',
          suggestion: 'Use AuditFieldsUtil.prepareInsertParams() for clean audit field addition',
          example: `
            // Before:
        ...
        },
        {
          ...
          suggestion: 'Consider using AuditFieldsUtil.createInsertWithAudit() for SQL generation',
          example: `
