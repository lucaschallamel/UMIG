...
describe('Controls Generator (07_generate_controls.js)', () => {
  ...
  describe('generateControls', () => {
    it('should call eraseControlsTables when erase option is true', async () => {
      ...
      expect(client.query).toHaveBeenCalledWith('TRUNCATE TABLE "controls_master_ctm" RESTART IDENTITY CASCADE');
    });

    it('should throw an error if no master phases are found', async () => {
      client.query.mockResolvedValue({ rows: [] }); // No phases
      await expect(generateControls(CONFIG)).rejects.toThrow('Cannot generate controls: No master phases found in the database.');
    });
    ...
    it('should generate the correct number of controls with correct data', async () => {
      ...
      const insertCalls = client.query.mock.calls.filter((call) => call[0].text?.includes('INSERT INTO controls_master_ctm'));
