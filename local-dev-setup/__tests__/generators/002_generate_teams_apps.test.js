...
describe("Data Generator: Teams and Applications", () => {
  ...
  describe("generateTeams", () => {
    test("it should generate the correct number of teams", async () => {
      ...
      const teamInsertions = client.query.mock.calls.filter((call) =>
        call[0].includes("INSERT INTO teams_tms"),
      );
    ...
    test("it should always create the IT_CUTOVER team first", async () => {
      await generateTeams(client, config);
      const firstTeamCall = client.query.mock.calls.find((call) =>
        call[0].includes("INSERT INTO teams_tms"),
      );
    ...
    test("it should generate teams with new, more realistic names", async () => {
      await generateTeams(client, config);
      const teamCalls = client.query.mock.calls.filter((call) =>
        call[0].includes("INSERT INTO teams_tms"),
      );
  ...
  describe("generateApplications", () => {
    test("it should generate the correct number of applications", async () => {
      ...
      const appInsertions = client.query.mock.calls.filter((call) =>
        call[0].includes("INSERT INTO applications_app"),
      );
  ...
  describe("eraseTeamsAndAppsTables", () => {
    test("it should truncate all related tables", async () => {
      ...
      expect(client.query).toHaveBeenCalledWith(
        'TRUNCATE TABLE "teams_tms_x_applications_app" RESTART IDENTITY CASCADE',
      );
      expect(client.query).toHaveBeenCalledWith(
        'TRUNCATE TABLE "teams_tms" RESTART IDENTITY CASCADE',
      );
      expect(client.query).toHaveBeenCalledWith(
        'TRUNCATE TABLE "applications_app" RESTART IDENTITY CASCADE',
      );
    });
  });
  ...
  describe("generateTeamsAndApps (Orchestrator)", () => {
    test("it should throw an error if no teams or apps exist for linking", async () => {
      client.query.mockImplementation((query) => {
        if (query.includes("SELECT tms_id FROM teams_tms"))
          return Promise.resolve({ rows: [] });
        if (query.includes("SELECT app_id FROM applications_app"))
          return Promise.resolve({ rows: [] });
        return Promise.resolve({ rows: [] });
      });
    ...
    test("it should create the exact number of links specified by the mock", async () => {
      ...
      client.query.mockImplementation((query) => {
        if (query.startsWith("SELECT tms_id"))
          return Promise.resolve({ rows: mockTeams });
        if (query.startsWith("SELECT app_id"))
          return Promise.resolve({ rows: mockApps });
        return Promise.resolve({ rows: [] });
      });

      faker.number.int.mockReturnValue(linksPerTeam);
      faker.helpers.arrayElements.mockImplementation((arr, count) =>
        arr.slice(0, count),
      );

      await generateTeamsAndApps(config);

      const linkInsertCalls = client.query.mock.calls.filter((call) =>
        call[0].includes("INSERT INTO teams_tms_x_applications_app"),
      );
    ...
    test("it should call erase when the erase option is true", async () => {
      ...
      jest
        .spyOn(faker.helpers, "arrayElements")
        .mockReturnValue([{ app_id: 101 }]);
    ...
    test("it should handle and throw an error if table erase fails", async () => {
      ...
      await expect(
        generateTeamsAndApps(config, { erase: true }),
      ).rejects.toThrow(dbError);
      expect(console.error).toHaveBeenCalledWith(
        "Error generating teams and applications:",
        dbError,
      );
    });
  });
