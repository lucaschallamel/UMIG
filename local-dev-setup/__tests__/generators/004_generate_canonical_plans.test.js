...
import {
  generateCanonicalPlans,
  eraseCanonicalPlanTables,
...
describe("Canonical Plans Generator (04_generate_canonical_plans.js)", () => {
  ...
  const mockAllMasterData = (client) => {
    client.query.mockImplementation((sql) => {
      if (sql.includes("FROM teams_tms"))
        return Promise.resolve({ rows: mockTeams });
      if (sql.includes("FROM step_types_stt"))
        return Promise.resolve({ rows: mockStepTypes });
      if (sql.includes("FROM environment_roles_enr"))
        return Promise.resolve({ rows: mockEnvRoles });
      if (sql.includes("FROM iteration_types_itt"))
        return Promise.resolve({ rows: mockIterationTypes });
      if (
        sql.includes(
      ...
      if (sql.startsWith("INSERT INTO"))
        return Promise.resolve({
          rows: [{ plm_id: 1, sqm_id: 1, phm_id: 1, stm_id: 1 }],
        });
  ...
  describe("generateCanonicalPlans", () => {
    it("should call eraseCanonicalPlanTables when erase option is true", async () => {
      ...
      const truncateCalls = client.query.mock.calls.filter((call) =>
        call[0].startsWith("TRUNCATE"),
      );
      expect(truncateCalls.length).toBe(6);
      expect(truncateCalls[0][0]).toContain(
    ...
    it("should throw an error if master data is missing", async () => {
      const expectedError =
        "Cannot generate plans: Missing master data (teams, step types, environment roles, iteration types, or plan statuses).";
      ...
      await expect(generateCanonicalPlans(CONFIG, {})).rejects.toThrow(
        expectedError,
      );
    ...
    it("should generate a full hierarchy of canonical plans and populate join tables", async () => {
      ...
      client.query.mockImplementation((sql) => {
        ...
        if (
          sql.startsWith("INSERT INTO steps_master_stm_x_teams_tms_impacted")
        ) {
          inserts.steps_x_teams++;
          return Promise.resolve({ rows: [] });
        }
        if (
          sql.startsWith("INSERT INTO steps_master_stm_x_iteration_types_itt")
        ) {
      ...
      const expectedPlans =
        CONFIG.CANONICAL_PLANS.PER_MIGRATION * CONFIG.MIGRATIONS.COUNT;
  ...
  describe("eraseCanonicalPlanTables", () => {
    it("should truncate all canonical plan tables in the correct order", async () => {
      ...
      expect(calls[0]).toBe(
        'TRUNCATE TABLE "steps_master_stm_x_teams_tms_impacted" RESTART IDENTITY CASCADE',
      );
      expect(calls[1]).toBe(
        'TRUNCATE TABLE "steps_master_stm_x_iteration_types_itt" RESTART IDENTITY CASCADE',
      );
      expect(calls[2]).toBe(
        'TRUNCATE TABLE "steps_master_stm" RESTART IDENTITY CASCADE',
      );
      expect(calls[3]).toBe(
        'TRUNCATE TABLE "phases_master_phm" RESTART IDENTITY CASCADE',
      );
      expect(calls[4]).toBe(
        'TRUNCATE TABLE "sequences_master_sqm" RESTART IDENTITY CASCADE',
      );
      expect(calls[5]).toBe(
        'TRUNCATE TABLE "plans_master_plm" RESTART IDENTITY CASCADE',
      );
    ...
    it("should throw an error if truncation fails", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        `Error erasing canonical plan tables: ${dbError}`,
      );
