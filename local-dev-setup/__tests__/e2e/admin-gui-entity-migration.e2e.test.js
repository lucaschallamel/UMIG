...
test.describe("Admin GUI Entity Migration E2E", () => {
  ...
  test.describe("Complete User Workflow - Iteration Types", () => {
    test("should complete full lifecycle: Create → Edit → Delete Iteration Type", async ({
      page,
    }) => {
      ...
      const createSuccess = await page
      ...
      const newRow = await page
        .locator('#iterationTypes-table tbody tr:has-text("E2ETEST")')
        .isVisible();
      expect(newRow).toBe(true);

      // Step 3: Edit the created item
      await page.click(
        '#iterationTypes-table tbody tr:has-text("E2ETEST") .btn-edit',
      );
      await page.waitForSelector(".modal", { timeout: 3000 });
      ...
      const updateSuccess = await page
        .locator(".notification-success")
        .isVisible();
      ...
      // Click delete button
      await page.click(
        '#iterationTypes-table tbody tr:has-text("E2ETEST") .btn-delete',
      );

      // Handle confirmation dialog if it appears
      try {
        await page.waitForSelector(".confirm-dialog, .modal", {
          timeout: 2000,
        });
      ...
      await page.waitForTimeout(2000);
      const deleteSuccess = await page
        .locator(".notification-success")
        .isVisible();
  ...
  test.describe("Complete User Workflow - Migration Types", () => {
    test("should complete full lifecycle: Create → Edit → Delete Migration Type", async ({
      page,
    }) => {
      let createdItem = null;
      ...
      // Fill form
      await page.fill("#mit_code", "MIGRATE01");
      await page.fill("#mit_name", "Migration Test 01");
      await page.fill("#mit_description", "E2E Migration test");
      await page.fill("#mit_color", "#9C27B0");
      await page.fill("#mit_icon", "migrate-test");
      ...
      // Edit the item
      await page.click(
        '#migrationTypes-table tbody tr:has-text("MIGRATE01") .btn-edit',
      );
      await page.waitForSelector(".modal", { timeout: 3000 });

      // Verify ID field is present and readonly in edit mode
      const idFieldInEdit = await page.locator("#mit_id").isVisible();
      expect(idFieldInEdit).toBe(true);

      const idFieldReadonly = await page
        .locator("#mit_id:disabled")
        .isVisible();
      expect(idFieldReadonly).toBe(true);

      // Update description
      await page.fill("#mit_description", "Updated E2E Migration test");
  ...
  test.describe("User Experience and Accessibility", () => {
    ...
    test("should provide appropriate ARIA labels and roles", async ({
      ...
      const tableRole = await page.getAttribute(
        "#iterationTypes-table",
        "role",
      );
      expect(tableRole).toBe("table");

      // Check button accessibility
      const addButtonLabel = await page.getAttribute(
        "#add-iterationTypes",
        "aria-label",
      );
    ...
    test("should handle responsive design for different screen sizes", async ({
      page,
    }) => {
  ...
  test.describe("Cross-browser Compatibility", () => {
    test("should work consistently across browser features", async ({
      page,
    }) => {
  ...
  test.describe("Performance and Load Testing", () => {
    ...
    test("should handle multiple modal operations efficiently", async ({
      page,
    }) => {
      await page.click('button[data-section="iterationTypes"]');
      await page.waitForSelector("#iterationTypes-table", { timeout: 5000 });
