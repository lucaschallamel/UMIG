...
describe('Teams Cross-Browser Compatibility', () => {
    ...
    BROWSERS.forEach(browserName => {
        ...
        describe(`${BROWSER_CONFIGS[browserName].name} Browser Tests`, () => {
            ...
            test(`should load Teams interface correctly in ${browserName}`, async ({ browser }) => {
                ...
                await expect(page.locator('.create-team-btn, button[aria-label*="Create"]')).toBeVisible();
            ...
            test(`should handle team creation workflow in ${browserName}`, async ({ browser }) => {
                const context = await browser.newContext();
                const page = await context.newPage();
                ...
                await page.fill('textarea[name="description"], #team-description', TEST_TEAM.description);
                
                // Select status if dropdown exists
                const statusSelect = page.locator('select[name="status"], #team-status');
                ...
                await expect(successMessage.or(teamInList)).toBeVisible({ timeout: 10000 });
            ...
            test(`should handle form validation consistently in ${browserName}`, async ({ browser }) => {
                ...
                const validationMessages = page.locator('.error-message, .field-error, [aria-invalid="true"]');
            ...
            test(`should support keyboard navigation in ${browserName}`, async ({ browser }) => {
                const context = await browser.newContext();
                const page = await context.newPage();
                ...
                const createButton = page.locator('.create-team-btn, button[aria-label*="Create"]');
            ...
            test(`should handle data table interactions in ${browserName}`, async ({ browser }) => {
                const context = await browser.newContext();
                const page = await context.newPage();
                ...
                const sortableHeaders = page.locator('th[aria-sort], th.sortable, button[aria-sort]');
                ...
                if (headerCount > 0) {
                    ...
                    const firstRowData = await page.locator('tbody tr:first-child td:first-child').textContent();
                ...
                if (await pagination.isVisible()) {
                    ...
                    if (await nextButton.isVisible() && await nextButton.isEnabled()) {
                        ...
                        // Verify page change
                        const pageIndicator = page.locator('.current-page, [aria-current="page"]');
            ...
            test(`should maintain performance standards in ${browserName}`, async ({ browser }) => {
                ...
                await page.click('.create-team-btn, button[aria-label*="Create"]');
                await page.waitForSelector('form, .modal', { state: 'visible', timeout: 3000 });
                ...
                if (await table.isVisible()) {
                    ...
                    // Try sorting
                    const sortButton = page.locator('th[aria-sort], th button, .sort-button').first();
    ...
    describe('Mobile Device Compatibility', () => {
        ...
        MOBILE_DEVICES.forEach(device => {
            ...
            test(`should work on ${device.name}`, async ({ browser }) => {
                ...
                const mobileMenu = page.locator('.mobile-menu, .hamburger, .nav-toggle');
    ...
    describe('CSS and Styling Consistency', () => {
        ...
        test('should handle CSS Grid and Flexbox layouts', async ({ browser }) => {
            ...
            const gridElements = page.locator('.grid, [style*="grid"], [class*="grid"]');
            ...
            const flexElements = page.locator('.flex, [style*="flex"], [class*="flex"]');
            if (await flexElements.count() > 0) {
                ...
                if (flexStyles.display.includes('flex')) {
                    expect(['row', 'column', 'row-reverse', 'column-reverse']).toContain(flexStyles.flexDirection);
                }
            }
        ...
        test('should support modern CSS features with fallbacks', async ({ browser }) => {
            const context = await browser.newContext();
            const page = await context.newPage();
            ...
            // Test CSS animations
            const animationElements = page.locator('[class*="animate"], [style*="animation"]');
            if (await animationElements.count() > 0) {
                const animationStyles = await animationElements.first().evaluate(el => {
                    const styles = window.getComputedStyle(el);
                    return {
    ...
    describe('Feature Detection and Polyfills', () => {
        ...
        test('should gracefully handle missing modern JavaScript features', async ({ browser }) => {
            const context = await browser.newContext();
            const page = await context.newPage();
        ...
        test('should detect browser capabilities correctly', async ({ browser }) => {
            const context = await browser.newContext();
            const page = await context.newPage();
