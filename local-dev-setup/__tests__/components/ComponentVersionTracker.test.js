...
const componentPath = path.join(
  __dirname,
  "../../../src/groovy/umig/web/js/components/ComponentVersionTracker.js",
);
const databaseManagerPath = path.join(
  __dirname,
  "../../../src/groovy/umig/web/js/components/DatabaseVersionManager.js",
);
...
if (!fs.existsSync(databaseManagerPath)) {
  throw new Error(
    `DatabaseVersionManager.js not found at ${databaseManagerPath}`,
  );
}
...
describe("ComponentVersionTracker", () => {
  ...
  describe("DatabaseVersionManager Integration", () => {
    test("should integrate with DatabaseVersionManager when available", async () => {
      ...
      expect(tracker.databaseVersionManager).toBeInstanceOf(
        global.DatabaseVersionManager,
      );
  ...
  describe("UI Component Version Detection", () => {
    ...
    test("should calculate component availability ratio correctly", async () => {
      ...
      const uiVersion = tracker.versionRegistries.ui.get("current");
      expect(uiVersion.availableComponents).toBeGreaterThan(0);
      expect(uiVersion.availableComponents).toBeLessThan(
        uiVersion.totalComponents,
      );
  ...
  describe("Compatibility Matrix Generation", () => {
    ...
    test("should calculate compatibility scores correctly", () => {
      ...
      const score = tracker.calculateCompatibilityScore(
        "api",
        semVerA,
        "database",
        semVerB,
    ...
    test("should detect breaking changes between major versions", () => {
      ...
      const breaking = tracker.detectBreakingChanges(
        "api",
        semVerA,
        "database",
        semVerB,
  ...
  describe("System Compatibility Analysis", () => {
    beforeEach(async () => {
      ...
      tracker.compatibilityMatrix.set("api-database", {
        ...
        status: "acceptable",
        issues: [
          { severity: "medium", description: "Minor compatibility issue" },
        ],
    ...
    test("should calculate correct average compatibility score", async () => {
      ...
      expect(tracker.systemState.compatibilityScore).toBeCloseTo(
        expectedScore,
        2,
      );
    });
  ...
  describe("Report Generation", () => {
    ...
    test("should include all version information in report", () => {
      ...
      expect(report).toContain(
        `API Version:      ${tracker.systemState.apiVersion}`,
      );
      expect(report).toContain(
        `UI Version:       ${tracker.systemState.uiVersion}`,
      );
      expect(report).toContain(
        `Backend Version:  ${tracker.systemState.backendVersion}`,
      );
      expect(report).toContain(
        `Database Version: ${tracker.systemState.databaseVersion}`,
      );
    });
  });
