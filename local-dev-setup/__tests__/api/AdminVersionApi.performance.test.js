...
describe('AdminVersionApi Performance Tests', () => {
    ...
    describe('/admin/version Performance', () => {
        test('should respond within 200ms target time', async () => {
            ...
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/version']);
        ...
        test('should respond within target with includeDetails=true', async () => {
            ...
            const response = await axiosClient.get('/adminVersion?includeDetails=true');

            const endTime = performance.now();
            const responseTime = endTime - startTime;

            console.log(`/admin/version (with details) response time: ${responseTime.toFixed(2)}ms`);

            expect(response.status).toBe(200);
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/version'] + 50); // Allow 50ms extra for details
            expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
        ...
        test('should handle multiple concurrent requests efficiently', async () => {
            ...
            console.log(`Average concurrent response time: ${averageResponseTime.toFixed(2)}ms`);

            expect(averageResponseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/version'] + 100); // Allow some overhead for concurrency
        });
    });
    ...
    describe('/admin/components Performance', () => {
        test('should respond within 300ms target time', async () => {
            ...
            console.log(`/admin/components response time: ${responseTime.toFixed(2)}ms`);
            ...
            // Performance assertions
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/components']);
            expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
        });
        ...
        test('should respond efficiently with specific component type filter', async () => {
            ...
            for (const componentType of componentTypes) {
                ...
                const response = await axiosClient.get(`/adminComponents?type=${componentType}`);

                const endTime = performance.now();
                const responseTime = endTime - startTime;

                console.log(`/admin/components (${componentType}) response time: ${responseTime.toFixed(2)}ms`);
        ...
        test('should include performance metrics efficiently', async () => {
            ...
            const response = await axiosClient.get('/adminComponents?includeMetrics=true&includeHealth=true');

            const endTime = performance.now();
            const responseTime = endTime - startTime;

            console.log(`/admin/components (with metrics) response time: ${responseTime.toFixed(2)}ms`);

            expect(response.status).toBe(200);
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/components'] + 50); // Allow extra time for metrics
            expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
    ...
    describe('/admin/compatibility Performance', () => {
        test('should respond within 400ms target time for full matrix', async () => {
            ...
            console.log(`/admin/compatibility (full matrix) response time: ${responseTime.toFixed(2)}ms`);
            ...
            // Performance assertions
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/compatibility']);
            expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
        });
        ...
        test('should respond efficiently for specific component compatibility', async () => {
            ...
            for (const { source, target } of componentPairs) {
                ...
                const response = await axiosClient.get(`/adminCompatibility?source=${source}&target=${target}`);

                const endTime = performance.now();
                const responseTime = endTime - startTime;

                console.log(`/admin/compatibility (${source}->${target}) response time: ${responseTime.toFixed(2)}ms`);

                expect(response.status).toBe(200);
                expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/compatibility'] - 100); // Should be faster for specific pairs
                expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
        ...
        test('should handle complex analysis options within time limits', async () => {
            ...
            const response = await axiosClient.get('/adminCompatibility?includeUpgradePaths=true&includeBreakingChanges=true');

            const endTime = performance.now();
            const responseTime = endTime - startTime;

            console.log(`/admin/compatibility (full analysis) response time: ${responseTime.toFixed(2)}ms`);
    ...
    describe('/admin/build-info Performance', () => {
        test('should respond within 250ms target time', async () => {
            ...
            console.log(`/admin/build-info response time: ${responseTime.toFixed(2)}ms`);
            ...
            // Performance assertions
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/build-info']);
            expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
        });
        ...
        test('should respond efficiently with all information included', async () => {
            ...
            const response = await axiosClient.get('/adminBuildInfo?includePackages=true&includeEnvironment=true&includeResources=true');

            const endTime = performance.now();
            const responseTime = endTime - startTime;

            console.log(`/admin/build-info (comprehensive) response time: ${responseTime.toFixed(2)}ms`);

            expect(response.status).toBe(200);
            expect(responseTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/build-info'] + 50); // Allow extra time for comprehensive data
            expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);
        ...
        test('should provide deployment readiness assessment efficiently', async () => {
            ...
            const response = await axiosClient.get('/adminBuildInfo?includePackages=true');

            const endTime = performance.now();
            const responseTime = endTime - startTime;

            console.log(`/admin/build-info (deployment focus) response time: ${responseTime.toFixed(2)}ms`);
    ...
    describe('Cross-Endpoint Performance Analysis', () => {
        test('all endpoints should meet their individual performance targets', async () => {
            const endpoints = [
                { path: '/adminVersion', threshold: PERFORMANCE_THRESHOLDS['/admin/version'] },
                { path: '/adminComponents', threshold: PERFORMANCE_THRESHOLDS['/admin/components'] },
                { path: '/adminCompatibility', threshold: PERFORMANCE_THRESHOLDS['/admin/compatibility'] },
                { path: '/adminBuildInfo', threshold: PERFORMANCE_THRESHOLDS['/admin/build-info'] }
            ];
            ...
            for (const { path, threshold } of endpoints) {
                ...
                console.log(`${path} performance: ${responseTime.toFixed(2)}ms (target: <${threshold}ms)`);
            ...
            console.log(`Average endpoint response time: ${averageTime.toFixed(2)}ms`);
        ...
        test('endpoints should handle concurrent access efficiently', async () => {
            const endpoints = ['/adminVersion', '/adminComponents', '/adminCompatibility', '/adminBuildInfo'];
            const promises = [];
            ...
            results.forEach(({ endpoint, response, responseTime }) => {
                console.log(`Concurrent ${endpoint} response time: ${responseTime.toFixed(2)}ms`);

                expect(response.status).toBe(200);
                expect(responseTime).toBeLessThan(MAX_RESPONSE_TIME);

                // Individual endpoint should still meet its threshold with reasonable overhead
                const threshold = PERFORMANCE_THRESHOLDS[endpoint.replace('admin', '/admin/')];
            ...
            const totalConcurrentTime = Math.max(...results.map(r => r.responseTime));
            console.log(`Total concurrent execution time: ${totalConcurrentTime.toFixed(2)}ms`);
        ...
        test('should maintain performance under repeated load', async () => {
            ...
            expect(averageTime).toBeLessThan(PERFORMANCE_THRESHOLDS['/admin/version'] + 50);
    ...
    describe('Error Handling Performance', () => {
        test('should handle timeout gracefully and quickly', async () => {
            ...
            try {
                const response = await axiosClient.get('/adminCompatibility?source=invalid&target=invalid');
        ...
        test('should provide meaningful error responses quickly', async () => {
            ...
            for (const malformedRequest of malformedRequests) {
                ...
                try {
                    ...
                    console.log(`${malformedRequest} response time: ${responseTime.toFixed(2)}ms`);
                    ...
                    const responseTime = performance.now() - startTime;
                    console.log(`${malformedRequest} error response time: ${responseTime.toFixed(2)}ms`);
