...
test.describe('Admin GUI Performance', () => {
  ...
  test('iframe isolation mode should load faster', async ({ page }) => {
    ...
    await page.goto('/confluence/pages/viewpage.action?pageId=12345&umig_iframe=true#umig-admin-gui');
  ...
  test('delayed loading mode should wait appropriately', async ({ page }) => {
    await page.goto('/confluence/pages/viewpage.action?pageId=12345&umig_delay=true#umig-admin-gui');
    
    // Check that loader appears first
    await expect(page.locator('#umig-delayed-loader')).toBeVisible();
    
    // Wait for actual content
    await page.waitForSelector('#umig-admin-gui-root', { state: 'visible', timeout: 5000 });
