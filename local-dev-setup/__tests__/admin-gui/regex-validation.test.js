...
test.describe('Regex Pattern Validation', () => {
  ...
  test('should validate Confluence warning patterns', async ({ page }) => {
    ...
    const patternResults = await page.evaluate(() => {
      ...
      testMessages.forEach(msg => {
        results[msg] = confluencePatterns.some(pattern => msg.includes(pattern));
      });
    ...
    expect(patternResults['AJS.params is deprecated and will be removed']).toBe(true);
    expect(patternResults['AJS.debounce function is deprecated']).toBe(true);
    expect(patternResults['Dialog is deprecated, use modern dialogs']).toBe(true);
  ...
  test('should preserve UMIG messages in console suppression', async ({ page }) => {
    let consoleMessages = [];
    ...
    const umigMessages = consoleMessages.filter(msg => msg.includes('[UMIG]'));
    expect(umigMessages.length).toBeGreaterThan(0);
    
    // Verify we're getting our performance messages
    const performanceMessages = umigMessages.filter(msg => 
