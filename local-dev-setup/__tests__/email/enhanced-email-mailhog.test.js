...
describe("US-039: Enhanced Email Notifications - MailHog Integration", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      "Testing mobile-responsive email templates with comprehensive validation",
    );
    console.log("=".repeat(70));
  ...
  afterAll(() => {
    ...
    console.log(
      `MailHog Connectivity: ${testResults.connectivity ? "✅ Connected" : "❌ Failed"}`,
    );
    ...
    const emailTemplateTypes = [
      {
        ...
        color: "blue",
        description:
          "Step status change notification with mobile responsiveness",
      },
      {
        type: "STEP_OPENED",
        color: "green",
    ...
    emailTemplateTypes.forEach(({ type, color, description }) => {
      test(`should send and validate ${type} enhanced email template`, async () => {
        ...
        if (testEmail?.Content?.Body) {
          ...
          console.log(
            `   ✅ Email contains required mobile-responsive elements`,
          );
      ...
      const mobileFeatures = [
        {
          ...
          check: () => emailBody.includes("viewport"),
        },
        {
          feature: "Table-based Layout",
          check: () =>
            emailBody.includes("<table") && emailBody.includes('width="600"'),
        },
        {
          feature: "Touch-friendly Buttons",
          check: () =>
            emailBody.includes("44px") || emailBody.includes("min-height"),
        },
        {
    ...
    test("should validate step content rendering in emails", async () => {
      ...
      const contentElements = [
        ...
        { element: "Step Code", check: () => emailBody.includes("TST-001") },
        {
          element: "Step Status",
          check: () =>
            emailBody.includes("COMPLETED") ||
            emailBody.includes("IN_PROGRESS"),
        },
        {
          element: "Instructions List",
          check: () =>
            emailBody.includes("instruction") &&
            (emailBody.includes("<li>") || emailBody.includes("•")),
        },
        {
          element: "Team Assignment",
          check: () =>
            emailBody.includes("Test Team") || emailBody.includes("Assigned"),
        },
        {
          element: "Confluence URL",
          check: () =>
      ...
      contentElements.forEach(({ element, check }) => {
        ...
        console.log(
          `   ${hasElement ? "✅" : "⚠️"} ${element}: ${hasElement ? "Present" : "Not detected"}`,
        );
      });
    ...
    test("should validate URL construction and security", async () => {
      ...
      if (urls.length > 0) {
        ...
        urls.forEach((url, index) => {
          console.log(
            `   🔗 URL ${index + 1}: ${url.substring(0, 80)}${url.length > 80 ? "..." : ""}`,
          );
  ...
  describe("Email Client Compatibility Checks", () => {
    test("should validate HTML structure for email client compatibility", async () => {
      ...
      const compatibilityChecks = [
        {
          ...
          test: () =>
            emailBody.includes("<!doctype html>") ||
            emailBody.includes("<!DOCTYPE"),
        },
        {
          check: "Table-based layout",
          test: () =>
            emailBody.includes("<table") &&
            emailBody.includes("<tr") &&
            emailBody.includes("<td"),
        },
        {
          check: "Inline CSS styles",
          test: () => emailBody.includes("style="),
        },
        {
          check: "MSO conditional comments",
          test: () =>
            emailBody.includes("<!--[if mso]-->") ||
            emailBody.includes("[if gte mso"),
        },
        {
          check: "Fallback fonts",
          test: () =>
            emailBody.includes("Arial") || emailBody.includes("sans-serif"),
        },
        {
          check: "Image alt attributes",
          test: () => !emailBody.includes("<img") || emailBody.includes("alt="),
        },
        {
          check: "Proper encoding",
          test: () =>
            emailBody.includes("UTF-8") || emailBody.includes("charset"),
        },
      ];
      ...
      compatibilityChecks.forEach(({ check, test }) => {
        ...
        console.log(
          `   ${passes ? "✅" : "⚠️"} ${check}: ${passes ? "Present" : "Missing"}`,
        );
      });
    ...
    test("should verify cross-platform rendering elements", async () => {
      ...
      const platformElements = [
        {
          ...
          elements: ["viewport", "max-width", "@media", "touch-friendly"],
        },
        {
          platform: "Desktop (Outlook)",
          elements: ["mso", "VML", "table", "cellpadding"],
        },
        {
          platform: "Web Clients (Gmail)",
          elements: ["table", "inline", "style", "sans-serif"],
        },
      ];
      ...
      platformElements.forEach(({ platform, elements }) => {
        ...
        const score = Math.round(
          (detectedElements.length / elements.length) * 100,
        );
        console.log(
          `   ${platform}: ${score}% compatibility (${detectedElements.length}/${elements.length} elements)`,
        );
  ...
  describe("Performance and Health Metrics", () => {
    test("should measure email generation and delivery performance", async () => {
      ...
      console.log(
        `   ✅ Email delivered successfully within performance requirements`,
      );
    });
    ...
    test("should validate system health check endpoints", async () => {
      ...
      try {
        ...
        console.log(
          `   📡 MailHog Service: ${mailhogHealth ? "✅ Healthy" : "❌ Unhealthy"}`,
        );

        // Test email template availability
        const templateExists = await checkEmailTemplateExists();
        console.log(
          `   📄 Email Template: ${templateExists ? "✅ Available" : "❌ Missing"}`,
        );
  ...
  async function clearMailHogInbox() {
    const { stdout } = await execAsync(
      `curl -X DELETE ${mailhogUrl}/api/v1/messages`,
    );
    return stdout;
  }
  ...
  async function sendEnhancedTestEmail(emailType, stepData) {
    try {
      ...
      testResults.errors.push(
        `Failed to send enhanced test email: ${error.message}`,
      );
  ...
  async function checkEmailTemplateExists() {
    try {
      const fs = require("fs");
      const templatePath =
        "/Users/lucaschallamel/Documents/GitHub/UMIG/src/groovy/umig/web/enhanced-mobile-email-template.html";
